...
public:
    ...
    bool ProcessMoveCommand(int drone_id, double ue_x, double ue_y, double ue_z,
                            ...
                            bool coalesce_queued_vertical_targets = false,
                            bool coalesce_queued_manual_targets = false);
    bool ProcessMoveCommandNed(int drone_id, double ned_n, double ned_e, double ned_d,
                               float speed_mps = 0.0f);
    /** Cancel pending movement and command a hold at the latest measured NED position. */
    bool AbortAndHoldAtCurrentPosition(int drone_id);
