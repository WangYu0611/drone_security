...
float normalize_requested_speed(float speed_mps)
{
    constexpr float kMinLimitedSpeedMps = 0.5f;
    constexpr float kMaxLimitedSpeedMps = 15.0f;
    if (!std::isfinite(speed_mps) || std::abs(speed_mps) < 0.001f) {
        return 0.0f;
    }
    return std::clamp(speed_mps, kMinLimitedSpeedMps, kMaxLimitedSpeedMps);
}

DroneControlPacket make_packet(int slot, double ned_n, double ned_e,
	                           double ned_d, uint32_t mode, float speed_mps = 0.0f)
{
    ...
    cmd.mode = mode;
    cmd.speed_mps = mode == 1 ? normalize_requested_speed(speed_mps) : 0.0f;
    // Jetson uses a NaN marker for its legacy position-control (unlimited)
    // mode.  Emit it explicitly for every move with no finite speed.
    cmd.use_unlimited_speed = mode == 1 && cmd.speed_mps == 0.0f;
    cmd.slot = slot;
    return cmd;
}
...
bool DroneManager::ProcessMoveCommand(int drone_id, double ue_x, double ue_y, double ue_z,
                                      float speed_mps,
                                      bool coalesce_queued_vertical_targets,
                                      bool coalesce_queued_manual_targets)
{
    std::lock_guard<std::mutex> lock(drones_mutex_);
    auto* ctx = GetContextUnsafe(drone_id);
    if (!ctx) return false;
    ...
    const auto packet = make_packet(ctx->slot, ned_n, ned_e, ned_d, 1, speed_mps);
    if (coalesce_queued_manual_targets) {
        ctx->command_queue->PushLatestManualTarget(packet);
    } else if (coalesce_queued_vertical_targets) {
        ctx->command_queue->PushLatestVerticalTarget(packet);
    } else {
        ctx->command_queue->Push(packet);
    }
    spdlog::info(
        "MoveCommand drone={} sequence={}: UE-relative(cm)=({:.2f},{:.2f},{:.2f}) "
        "-> power-on-relative NED(m)=({:.3f},{:.3f},{:.3f}), speed={}m/s",
        drone_id, packet.sequence, ue_x, ue_y, ue_z, ned_n, ned_e, ned_d,
        packet.speed_mps > 0.0f ? std::to_string(packet.speed_mps) : std::string("unlimited"));
    return true;
}
                                         ...
                                         float speed_mps)
{
    ...
    spdlog::info(
        ...
        "NED(m)=({:.3f},{:.3f},{:.3f}), speed={}m/s",
        drone_id, packet.sequence, ned_n, ned_e, ned_d,
        packet.speed_mps > 0.0f ? std::to_string(packet.speed_mps) : std::string("unlimited"));
    return true;
}

bool DroneManager::AbortAndHoldAtCurrentPosition(int drone_id)
{
    std::lock_guard<std::mutex> lock(drones_mutex_);
    auto* ctx = GetContextUnsafe(drone_id);
    if (!ctx) return false;

    // A queue stop must also discard commands that have not reached the heartbeat worker yet.
    // Otherwise a later resume/new task can replay an obsolete waypoint.
    ctx->command_queue->Clear();
    ctx->command_queue->SetPaused(false);

    // Never fall back to (0,0,0): that is the PX4 local origin, not a safe airborne hold point.
    if (!has_fresh_valid_local_position(*ctx)) {
        spdlog::warn("Abort drone {}: no fresh valid local telemetry, refusing to invent a hold target", drone_id);
        return false;
    }
    ...
    hb_manager_.RequestHold(drone_id, ctx->last_ned_x, ctx->last_ned_y, ctx->last_ned_z);
    spdlog::info(
        "Abort drone {}: cleared queued commands and holding measured power-on-relative NED({:.3f},{:.3f},{:.3f})",
...
std::vector<int> DroneManager::RefreshDisconnectedConnections()
{
    ...
    for (auto& [id, ctx] : drones_) {
        ...
        // Never start a heartbeat at a guessed local origin.  However, a
        // one-shot unconfirmed packet is safe and lets Jetson learn the
        // backend's current source IP, after which its normal telemetry can
        // establish an estimator-valid NED position.
        if (!has_fresh_valid_local_position(*ctx)) {
            if (hb_manager_.SendDiscoveryProbe(
                    id, ctx->slot, ctx->jetson_ip, ctx->send_port)) {
                refreshed_ids.push_back(id);
                spdlog::info(
                    "[ConnectionRefresh] sent one-shot discovery probe to drone {} -> {}:{}; "
                    "waiting for telemetry",
                    id, ctx->jetson_ip, ctx->send_port);
            } else {
                spdlog::warn(
                    "[ConnectionRefresh] discovery probe for drone {} could not be sent",
                    id);
            }
            continue;
        }
