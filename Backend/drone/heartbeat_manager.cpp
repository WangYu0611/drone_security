...
bool HeartbeatManager::SendDiscoveryProbe(int drone_id, int slot,
                                          const std::string& jetson_ip,
                                          int send_port)
{
    if (slot <= 0 || jetson_ip.empty() || send_port <= 0 || send_port > 65535) {
        spdlog::error(
            "[Heartbeat] Cannot send discovery probe for drone {}: slot={}, target={}:{}",
            drone_id, slot, jetson_ip, send_port);
        return false;
    }

    DroneControlPacket probe{};
    probe.sequence = NextControlSequence();
    probe.timestamp = static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::system_clock::now().time_since_epoch()).count())
        / 1000000.0;
    probe.slot = slot;
    probe.mode = 0;          // protocol-level hold; never starts a worker here
    probe.repeat_index = 1;  // one packet cannot satisfy the Jetson 3-packet default
    probe.repeat_total = 1;

    sender_.SetTarget(drone_id, jetson_ip, send_port);
    const bool sent = sender_.Send(drone_id, probe);
    spdlog::info(
        "[Heartbeat] Discovery probe drone {} slot {} -> {}:{} sent={} (single unconfirmed packet)",
        drone_id, slot, jetson_ip, send_port, sent);
    return sent;
}
