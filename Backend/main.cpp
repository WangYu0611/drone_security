...
int main(int argc, char* argv[])
{
    ...
    LatencyTester      latency_tester(udp_sender);
    ...
    exec_engine.SetMoveCallback([&](int drone_id, double ned_n, double ned_e, double ned_d, float speed_mps) {
        drone_mgr.ProcessMoveCommandNed(drone_id, ned_n, ned_e, ned_d, speed_mps);
    });
    ...
    // 8. 配置 UDP 控制目标（每个 slot 都可指向不同的 Jetson）。
    for (const auto& [slot, mapping] : config.port_map) {
        udp_sender.SetTarget(slot, mapping.jetson_host, mapping.send_port);
    }
    ...
    // 9b. 延迟探测 pong 回调 -> LatencyTester
    udp_receiver.SetPongCallback([&](int slot, uint64_t ping_id, double echoed_sent_at) {
        latency_tester.OnPong(slot, ping_id, echoed_sent_at);
    });
