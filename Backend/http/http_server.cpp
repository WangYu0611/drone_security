...
HttpServer::HttpServer(const AppConfig& config,
    ...
    , operational_context_((std::filesystem::path(config.storage_path).parent_path() / "operational_context.json").string())
    , security_plans_((std::filesystem::path(config.storage_path).parent_path() / "security_plans.json").string())
...
{
    const auto mockPath=std::filesystem::path(config.storage_path).parent_path()/"mock_execution.json";
    ...
    drone_mgr_.SetAlertCallback([this](int drone_id, const std::string& alert_type, int value) {
        auto msg = boost::json::object{
            {"type",       "alert"},
            {"alert_id", "ALERT-" + std::to_string(current_unix_seconds()) + "-" + std::to_string(drone_id)},
            {"drone_id",   get_number_or_string_id(drone_id)},
            {"drone_id_str", drone_id_string(drone_id)},
            {"alert",      alert_type},
...
void HttpServer::Run() {
    running_ = true;
    plan_lease_thread_=std::thread([this]{auto last=std::chrono::steady_clock::now();int leaseTick=0;while(running_) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        try {
            const auto now=std::chrono::steady_clock::now();const double dt=std::chrono::duration<double>(now-last).count();last=now;
            {std::lock_guard<std::mutex> transaction(plan_requests_mutex_);std::lock_guard<std::mutex> records(records_mutex_);
             std::set<std::string> uavs;for(const auto& d:drone_records_)if(d.slot>0)uavs.insert("UAV-"+std::string(d.slot<10?"0":"")+std::to_string(d.slot));
             security_plans_.tickExecutions(dt,uavs,[this](const boost::json::object& e){ws_manager_.broadcast(json_stringify(e));});}
            if(++leaseTick<10)continue;leaseTick=0;
            std::vector<std::shared_ptr<WsSession>> expired;
            { std::lock_guard<std::mutex> lock(context_clients_mutex_);
              for(const auto& entry:context_sessions_) {
                const auto& client=context_clients_.at(entry.first);
                auto seen=client.if_contains("last_seen");
                if(client.at("state")=="ONLINE" && seen && OperationalContext::now()-seen->to_number<double>()>12)
                    if(auto session=entry.second.lock()) expired.push_back(session);
              }
            }
            for(const auto& session:expired) {
                DisconnectContextClient(session);
                ws_manager_.send(session,"{\"type\":\"error\",\"message\":\"context heartbeat expired\"}");
            }
            security_plans_.heartbeat("",[this](const boost::json::object& e){ws_manager_.broadcast(json_stringify(e));});}
    ...
    http::request<http::string_body>& req)
{
    try {
        ...
        // Method/path only: acceptance audit excludes payloads and query credentials.
        spdlog::debug("[HTTP Request] {} {}", method, path);
        ...
        if(path=="/api/ui-preferences" || path=="/api/video-view") {
        ...
        if (method == "GET" && path == "/api/security-plans")
            return MakeResponse(req, 200, json_stringify(security_plans_.snapshot()));
        if (method == "POST" && path == "/api/security-plans") {
            try { return MakeResponse(req, 200, json_stringify(PlanRequest(require_object(body,"body")))); }
            catch (const PlanError& e) {
                return MakeResponse(req, 409, json_stringify(boost::json::object{{"code",e.code},{"params",e.params},{"message",e.what()},{"state",security_plans_.snapshot()}}));
            }
            catch (const ApiError& e) {
                return MakeResponse(req,e.status_code,json_stringify(boost::json::object{{"code",e.status_code==403?"ROLE_FORBIDDEN":"REQUEST_REJECTED"},{"params",boost::json::object{}},{"message",e.what()}}));
            }
        }

        if (method == "POST" && path == "/api/context/debug/alert") {
            if (!config_.debug) throw ApiError(403, "debug disabled");
            const auto& b = require_object(body, "body");
            const int drone = get_int(b, "drone_id");
            if (drone <= 0) throw ApiError(400, "invalid drone_id");
            boost::json::object event{{"type", "alert"}, {"alert_id", "ALERT-QA-" + std::to_string(current_unix_seconds())},
                {"drone_id", drone}, {"alert_type", "lost_connection"}, {"value", 0}};
            ws_manager_.broadcast(json_stringify(event));
            return MakeResponse(req, 200, json_stringify(event));
        }
        if (method == "GET" && path == "/api/context")
            return MakeResponse(req, 200, json_stringify(operational_context_.snapshot()));
        if (method == "GET" && path == "/api/context/clients")
            return MakeResponse(req, 200, json_stringify(ContextClients()));
        if (method == "POST" && path == "/api/context/clients")
            return MakeResponse(req, 200, json_stringify(RegisterContextClient(require_object(body, "body"))));
        if (method == "PATCH" && path == "/api/context")
            return MakeResponse(req, 200, json_stringify(UpdateContext(require_object(body, "body"))));
...
boost::json::value HttpServer::ApiStopArray(const std::string& id) {
    ...
    for (const auto& path : config.paths) {
        ...
        if (!drone_mgr_.AbortAndHoldAtCurrentPosition(drone_id)) {
            spdlog::warn("[HTTP] Stop array '{}': drone {} could not be held at a measured position",
                         id, drone_id);
        }
...
boost::json::value HttpServer::DebugMove(const std::string& id, const boost::json::object& body) {
    ...
    const float speed_mps = static_cast<float>(get_number(body, "speed", 0.0));
    if (!drone_mgr_.ProcessMoveCommand(drone_id, x, y, z, speed_mps))
        throw ApiError(404, "drone not found: " + id);
                                  ...
                                  const std::shared_ptr<WsSession>& session)
{
    ...
    if (msg.contains("mode")) {
        ...
        const float speed_mps = static_cast<float>(get_number(msg, "speed", 0.0));
        const bool coalesce_queued_vertical_targets = get_bool(msg, "coalesce_queued_vertical_targets", false);
        const bool coalesce_queued_manual_targets = get_bool(msg, "coalesce_queued_manual_targets", false);
        int drone_id = drone_id_from_string(drone_id_str);
        if (!drone_mgr_.ProcessMoveCommand(
                drone_id, x, y, z, speed_mps,
                coalesce_queued_vertical_targets, coalesce_queued_manual_targets)) {
            ws_manager_.send(session, json_stringify(boost::json::object{
                               ...
                               http::request<http::string_body> upgrade_req)
{
    ...
    { std::lock_guard<std::mutex> lock(session->write_mutex); session->alive = false; session->ws = nullptr; }
...
void HttpServer::LoadDrones() {
    try {
        ...
        for (const auto& item : arr) {
            ...
            const std::string jetson_host = rec.ip.empty()
                ? (mapping != config_.port_map.end()
                    ? mapping->second.jetson_host
                    : config_.jetson_host)
                : rec.ip;
            rec.ip = jetson_host;
            drone_mgr_.AddDrone(drone_id, rec.slot, rec.name, jetson_host, rec.port);
            drone_records_.push_back(rec);
...
boost::json::value HttpServer::ContextClients() {
    std::lock_guard<std::mutex> lock(context_clients_mutex_);
    boost::json::array result;
    for (const auto& entry : context_clients_) result.push_back(entry.second);
    return result;
}
boost::json::value HttpServer::RegisterContextClient(const boost::json::object& body) {
    const auto role = get_string(body, "client_role");
    if (role != "Command" && role != "Map" && role != "Video") throw ApiError(400, "invalid client role");
    for (const char* key : {"client_id", "instance_id", "hostname", "app_version"}) {
        auto v = body.if_contains(key);
        if (!v || !v->is_string() || v->as_string().empty() || v->as_string().size() > 128)
            throw ApiError(400, "invalid client identity");
    }
    const auto instance = get_string(body, "instance_id");
    std::lock_guard<std::mutex> lock(context_clients_mutex_);
    auto existing = context_clients_.find(instance);
    if (existing != context_clients_.end()) {
        if (existing->second.at("client_role") != body.at("client_role") || existing->second.at("client_id") != body.at("client_id"))
            throw ApiError(409, "instance identity conflict");
        return existing->second;
    }
    // Registration is not proof of a live event subscription.
    boost::json::object client;
    for (const char* key : {"client_id", "client_role", "instance_id", "hostname", "app_version"}) client[key] = body.at(key);
    client["connected_at"] = nullptr; client["last_seen"] = 0; client["hydrated"] = false; client["state"] = "OFFLINE"; client["hydrated"] = false; client["client_version"] = ++context_client_version_;
    context_clients_[instance] = client;
    return client;
}
boost::json::value HttpServer::UpdateContext(const boost::json::object& body) {
    std::string source, role;
    {
        std::lock_guard<std::mutex> lock(context_clients_mutex_);
        const auto instance = get_string(body, "instance_id");
        auto it = context_clients_.find(instance);
        if (it == context_clients_.end() || it->second.at("state") != "ONLINE") throw ApiError(409, "client not subscribed");
        source = get_string(it->second, "client_id"); role = get_string(it->second, "client_role");
    }
    auto item = body.if_contains("patch");
...
bool HttpServer::HandleContextMessage(const boost::json::object& body, const std::shared_ptr<WsSession>& session) {
    const auto type = get_string(body, "type");
    if (type == "context_ping") {
        std::string owner;
        ...
        if(!owner.empty()) {
            boost::json::object refreshed;
            { std::lock_guard<std::mutex> lock(context_clients_mutex_);
              auto& client=context_clients_.at(owner);
              client["last_seen"]=OperationalContext::now();
              const auto ready=body.if_contains("hydrated");
              client["hydrated"]=ready && ready->is_bool() && ready->as_bool();
              client["client_version"]=++context_client_version_; refreshed=client; }
            ws_manager_.broadcast(json_stringify(boost::json::object{{"type","client_connected"},{"payload",refreshed}}));
        }
    ...
    if (type != "subscribe_context") return false;
    boost::json::object client;
    {
        std::lock_guard<std::mutex> lock(context_clients_mutex_);
        const auto instance = get_string(body, "instance_id");
        auto it = context_clients_.find(instance);
        if (it == context_clients_.end()) throw ApiError(409, "register before subscribing");
        // Bind one identity to one active socket; replacing a dead connection is allowed.
        auto old = context_sessions_[instance].lock();
        if (old && old != session && old->alive) throw ApiError(409, "instance already online");
        for (const auto& entry : context_sessions_) if (entry.second.lock() == session && entry.first != instance)
            throw ApiError(409, "socket already bound");
        context_sessions_[instance] = session;
        it->second["client_version"] = ++context_client_version_;
        it->second["state"] = "ONLINE"; it->second["connected_at"] = OperationalContext::now();
        it->second["last_seen"] = OperationalContext::now(); it->second["hydrated"] = false;
        client = it->second;
    }
    ws_manager_.broadcast(json_stringify(boost::json::object{{"type", "client_connected"}, {"event_type", "client_connected"},
        {"event_id", "connected-" + get_string(client, "instance_id") + "-" + std::to_string(OperationalContext::now())},
        {"timestamp", OperationalContext::now()}, {"payload", client}}));
    // Subscribe before sampling: full-state events + version rejection close the snapshot/WS race.
    ws_manager_.send(session, json_stringify(boost::json::object{{"type", "context_subscribed"},
        {"payload", operational_context_.snapshot()}, {"clients", ContextClients()}}));
    return true;
}
void HttpServer::DisconnectContextClient(const std::shared_ptr<WsSession>& session) {
    boost::json::array disconnected;
    {
        std::lock_guard<std::mutex> lock(context_clients_mutex_);
        for (auto& entry : context_sessions_) if (entry.second.lock() == session) {
            auto& client = context_clients_.at(entry.first); client["state"] = "OFFLINE"; client["hydrated"] = false; client["client_version"] = ++context_client_version_;
            disconnected.push_back(client); entry.second.reset();
        }
    }
    for (const auto& client : disconnected)
        ws_manager_.broadcast(json_stringify(boost::json::object{{"type", "client_disconnected"}, {"event_type", "client_disconnected"},
            {"event_id", "disconnected-" + get_string(client.as_object(), "instance_id") + "-" + std::to_string(OperationalContext::now())},
            {"timestamp", OperationalContext::now()}, {"payload", client}}));
}

boost::json::value HttpServer::PlanRequest(const boost::json::object& body) {
    std::lock_guard<std::mutex> transaction(plan_requests_mutex_);
    std::string role, source;
    {
        std::lock_guard<std::mutex> lock(context_clients_mutex_);
        auto it=context_clients_.find(get_string(body,"instance_id"));
        if(it==context_clients_.end() || it->second.at("state")!="ONLINE") throw ApiError(409,"client not subscribed");
        role=get_string(it->second,"client_role"); source=get_string(it->second,"client_id");
    }
    ...
    auto select=[&](const std::string& plan,const std::string& mission) {
        ...
        return operational_context_.update({{"active_security_plan_id",plan.empty()?boost::json::value(nullptr):boost::json::value(plan)},
            {"active_mission_id",mission.empty()?boost::json::value(nullptr):boost::json::value(mission)}},source,
            [this](const boost::json::object& e){ws_manager_.broadcast(json_stringify(e));});
    };
    ...
    if(action=="select") return boost::json::object{{"context",select(get_string(body,"plan_id"),get_string(body,"mission_id"))},{"state",security_plans_.snapshot()}};
    if(action=="open_plan")security_plans_.checkSelection(get_string(body,"plan_id"),get_string(body,"mission_id"));
    // Hold the existing descriptor lock through validation and commit.
    std::lock_guard<std::mutex> records(records_mutex_);
    std::set<std::string> uavs; for(const auto& d:drone_records_) {
        // P1/UE canonical selection uses the existing numeric slot, while legacy records may use d1.
        if(d.slot>0)uavs.insert("UAV-"+std::string(d.slot<10?"0":"")+std::to_string(d.slot));
    }
    auto result=security_plans_.transact(body,uavs,source,[this](const boost::json::object& e){ws_manager_.broadcast(json_stringify(e));});
    if(action=="create_plan" || action=="add_mission" || action=="copy_plan" || action=="open_plan" || action=="delete_plan") result["context"]=select(get_string(result,"plan_id"),get_string(result,"mission_id"));
    if(action=="delete_mission") {
        auto context=operational_context_.snapshot();
        if(get_string(context,"active_mission_id")==get_string(body,"mission_id")) result["context"]=select(get_string(body,"plan_id"),"");
    }
    return result;
}
