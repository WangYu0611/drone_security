...
public:
    ...
    /// @param targets          集结目标点列表（输入输出，可能重分配 drone_id 和/或修改 Z）
    /// @param safety_radius_m  安全圆柱半径（米）
    /// @param bAllowTargetReassignment
    ///                         true: 根据当前位置重分配目标；false: 保留每个目标原有的
    ///                         drone_id，仅做冲突检测和高度分离。
    ///
    ...
    std::vector<PathConflict> Plan(
        ...
        double safety_radius_m,
        bool bAllowTargetReassignment = true) const;
