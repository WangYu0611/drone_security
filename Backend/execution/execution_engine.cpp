...
void ExecutionEngine::RunDroneTask(int drone_id)
{
    ...
    while (running_) {
        ...
        if (task.mode == "patrol") {
            ...
            if (has_override) {
                             ...
                             drone_id, ned_n, ned_e, ned_d);
                {
                    ...
                    target.ned_e = ned_e;
                    target.ned_d = ned_d;
                    target.speed_mps = override_wp.segment_speed;
                    target.avoidance_active = false;
                    target.avoidance_restore_at = {};
                    target.avoidance_cooldown_until = {};
                }
                if (move_cb_) move_cb_(drone_id, ned_n, ned_e, ned_d, override_wp.segment_speed);
...

        {
            ...
            target.ned_e = ned_e;
            target.ned_d = ned_d;
            target.speed_mps = wp.segment_speed;
            target.avoidance_active = false;
            target.avoidance_restore_at = {};
            target.avoidance_cooldown_until = {};
        }

        if (move_cb_) move_cb_(drone_id, ned_n, ned_e, ned_d, wp.segment_speed);
...
void ExecutionEngine::CheckAvoidance()
{
    ...
    for (size_t i = 0; i < states.size(); ++i) {
        for (size_t j = i + 1; j < states.size(); ++j) {
            ...
            if (move_cb_) {
                move_cb_(low_prio_id, applied.x, applied.y, applied.z, original.speed_mps);
            }
        }
    }
}
...
void ExecutionEngine::RestoreExpiredAvoidanceTargets(const std::chrono::steady_clock::time_point& now)
{
    ...
    std::vector<RestoreItem> items;
    {
        ...
        for (auto& [drone_id, target] : targets_) {
            ...
            item.speed_mps = target.speed_mps;
    ...
    for (const auto& item : items) {
        ...
        if (move_cb_) {
            move_cb_(item.drone_id, item.ned_n, item.ned_e, item.ned_d, item.speed_mps);
        }
