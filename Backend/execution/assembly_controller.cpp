...
bool AssemblyController::Start(const AssemblyConfig& config, double safety_cylinder_m)
{
    ...
    // ---- Step 2: 运行规划器（保留已确定的路径归属，只做 P2 冲突检测/高度分离） ----
    // The HTTP layer has already assigned every path a drone_id: manually matched paths
    // carry the operator's mapping, while auto-assigned paths carry the backend result.
    // Reassigning here would send a drone to another path's first waypoint and then make
    // it execute its original path after assembly.
    if (!drone_positions.empty() && safety_cylinder_m > 0) {
        auto conflicts = planner_.Plan(
            drone_positions, targets, safety_cylinder_m,
            /*bAllowTargetReassignment=*/false);
