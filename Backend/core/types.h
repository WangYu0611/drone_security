...
struct DroneControlPacket {
    ...
    // Requested 3D speed in m/s. 0 is the legacy/unlimited marker; limited
    // values are normalized to [0.5, 15] before entering the command queue.
    float    speed_mps = 0.0f;
    bool     use_unlimited_speed = false; // serialize speed as NaN for Jetson
    int      slot = 0;
...
struct PortMapping {
    ...
    int recv_port = 0;
    int send_port = 0;
    // Per-slot Jetson control target.  This deliberately lives beside the
    // ports: one backend can control a swarm whose Jetsons have different IPs.
    std::string jetson_host;
