...
AppConfig LoadConfig(const std::string& path)
{
    ...
    try {
        ...
        // --- jetson ---
        // Keep the single-host value as a backwards-compatible fallback for
        // older configurations that do not yet specify a host per slot.
        ...
        if (auto pm = root["port_map"]) {
            for (const auto& kv : pm) {
                ...
                mapping.jetson_host     = kv.second["host"].as<std::string>(cfg.jetson_host);
...
void ValidateConfig(const AppConfig& cfg)
{
    ...
    for (const auto& [slot, mapping] : cfg.port_map) {
        if (mapping.jetson_host.empty()) {
            throw std::invalid_argument(
                "host for slot " + std::to_string(slot) + " must not be empty");
        }
