...
> 本文件是后端唯一 README，已合并原 `README.md` 与 `README_backend.md` 的有效内容。详细字段以 `Docs/` 下的《接口与通讯数据规范》为准，坐标以《坐标系转换说明》为准。
...
port_map:
  1: { host: "192.168.30.103", send_port: 8889, recv_port: 8888, ros_topic_prefix: "/px4_1" }
  2: { host: "192.168.30.104", send_port: 8891, recv_port: 8890, ros_topic_prefix: "/px4_2" }
...
`port_map.<slot>.host` 是每架无人机的默认控制目标 IP；注册时传入的 `ip` 会覆盖该 slot 的默认值并写入注册表。`jetson.host` 仅保留为未填写 slot host 时的兼容默认值。
...
- `../Docs/接口与通讯数据规范.md`：HTTP、WebSocket、UDP JSON/YAML 字段定义。
- `../Docs/坐标系转换说明.md`：NED、UE5、GPS、Cesium 与上电原点。
- `../Docs/架构设计.md`：系统分层、时序与职责边界。
