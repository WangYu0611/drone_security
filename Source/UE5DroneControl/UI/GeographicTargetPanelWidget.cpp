...
void UGeographicTargetPanelWidget::ConfigureCommandLayout()
{
    bCommandLayout = true;
    CommandTheme::Drawer(WidgetTree);
    if (auto* Scroll = Cast<UScrollBox>(WidgetTree->FindWidget(TEXT("PanelScroll"))))
    ...
    if (PanelBody)
    {
        ...
        {
            BodySlot->SetPosition(FVector2D(-12, 52));
            BodySlot->SetSize(FVector2D(480, 420));
        }
    }
}
...
void UGeographicTargetPanelWidget::RefreshAvailability()
{
	...
	if (PanelBody)
	{
		if (UCanvasPanelSlot* BodySlot = Cast<UCanvasPanelSlot>(PanelBody->Slot))
		{
			...
			BodySlot->SetSize(bCommandLayout ? FVector2D(480.f, 420.f) : FVector2D(600.0f, DesiredHeight));
		}
	}
	if (BatchPathSection)
	{
