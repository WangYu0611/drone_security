...
void UDroneListItemWidget::ConfigureCommandSelection()
{
    bCommandSelection = true;
    CommandTheme::Text(DroneNameText, 17, CommandTheme::PrimaryText, true);
    CommandTheme::Text(StatusText, 11, CommandTheme::SecondaryText);
    CommandTheme::Text(SelectButtonText, 12, CommandTheme::PrimaryText, true);
    CommandTheme::Text(ConnectionStatusText, 11, CommandTheme::SecondaryText);
    CommandTheme::Button(NameLabelButton);
    CommandTheme::Button(CollapseButton);
    SetCollapsed(true);
    if (!CommandSummary && WidgetTree)
    {
        UBorder* Card = Cast<UBorder>(WidgetTree->RootWidget);
        UVerticalBox* Content = Card ? Cast<UVerticalBox>(Card->GetContent()) : nullptr;
        if (Content)
        {
            CommandSummary = WidgetTree->ConstructWidget<UTextBlock>();
            CommandTheme::Text(CommandSummary, 12, CommandTheme::SecondaryText);
            CommandSummary->SetAutoWrapText(true);
            Content->AddChild(CommandSummary);
        }
    }
    RefreshCommandSummary();
}
void UDroneListItemWidget::RefreshCommandSummary()
{
    if (!CommandSummary) return;
    UDroneRegistrySubsystem* Registry = GetGameInstance() ? GetGameInstance()->GetSubsystem<UDroneRegistrySubsystem>() : nullptr;
    FDroneTelemetrySnapshot Telemetry;
    FDroneTaskStateSnapshot Task;
    const bool bTelemetry = Registry && Registry->GetTelemetry(DroneId, Telemetry);
    const bool bTask = Registry && Registry->GetTaskState(DroneId, Task);
    const FString Battery = bTelemetry && Telemetry.BatteryPercent >= 0
        ? FString::Printf(TEXT("%d%%"), Telemetry.BatteryPercent) : TEXT("N/A");
    const FString TaskName = bTask ? StaticEnum<EDroneTaskState>()->GetDisplayNameTextByValue(static_cast<int64>(Task.State)).ToString() : TEXT("任务 N/A");
    CommandSummary->SetText(FText::Format(ProductText::Get(TEXT("Fleet.Summary")),FText::AsNumber(DroneId),FText::AsCultureInvariant(Battery),ProductText::Source(TaskName)));
}
void UDroneListItemWidget::OnCommandPrimaryChanged(int32 OldId, int32 NewId) { UpdateSelectionState(); }
...
void UDroneListItemWidget::SetDroneDataWithAvailability(const FString& Name, EDroneAvailability Availability)
{
    if (DroneNameText)
    {
        DroneNameText->SetText(ProductText::Source(Name));
    }
    ...
    if (StatusText)
    {
        ...
        case EDroneAvailability::Online:
            StatusText->SetText(ProductText::Source(TEXT("连接")));
            break;
        case EDroneAvailability::Lost:
            StatusText->SetText(ProductText::Source(TEXT("失联")));
            break;
        case EDroneAvailability::Offline:
        default:
            StatusText->SetText(ProductText::Source(TEXT("离线")));
            break;
        }
    }
...
void UDroneListItemWidget::NativeConstruct()
{
    ...
    if (SelectButton)
    {
        SelectButton->OnClicked.AddUniqueDynamic(this, &UDroneListItemWidget::OnSelectButtonClicked);
    }
    if (CollapseButton)
    {
        CollapseButton->OnClicked.AddUniqueDynamic(this, &UDroneListItemWidget::OnCollapseButtonClicked);
    }
    if (NameLabelButton)
    {
        NameLabelButton->OnClicked.AddUniqueDynamic(this, &UDroneListItemWidget::OnNameLabelButtonClicked);
    }
    if (ModelComboBox)
    {
        // 填充四个外观选项。BuildRuntimeCard 可能已被蓝图绑定的实例替代，
        // 故在此统一填充，且先清空避免 NativeConstruct 被重入时出现重复项。
        bSuppressModelComboCallback = true;
        ModelComboBox->ClearOptions();
        for (const EDroneModelType Type : GetAllDroneModelTypes())
        {
            ModelComboBox->AddOption(GetDroneModelDisplayName(Type));
        }
        bSuppressModelComboCallback = false;

        ModelComboBox->OnSelectionChanged.AddUniqueDynamic(this, &UDroneListItemWidget::OnModelComboChanged);
...
void UDroneListItemWidget::NativeDestruct()
{
    if (UGameInstance* GI = GetGameInstance())
    {
        if (UDroneRegistrySubsystem* Registry = GI->GetSubsystem<UDroneRegistrySubsystem>())
        {
            ...
            Registry->OnPrimarySelectionChanged.RemoveDynamic(this, &UDroneListItemWidget::OnCommandPrimaryChanged);
            Registry->OnDroneModelTypeChanged.RemoveDynamic(this, &UDroneListItemWidget::OnRegistryModelTypeChanged);
        }
    }
...
void UDroneListItemWidget::SetDroneId(int32 InDroneId)
{
    ...
    if (UGameInstance* GI = GetGameInstance())
    {
        if (UDroneRegistrySubsystem* Registry = GI->GetSubsystem<UDroneRegistrySubsystem>())
        {
            ...
            if (bCommandSelection) Registry->OnPrimarySelectionChanged.AddUniqueDynamic(this, &UDroneListItemWidget::OnCommandPrimaryChanged);

            Registry->OnDroneModelTypeChanged.RemoveDynamic(this, &UDroneListItemWidget::OnRegistryModelTypeChanged);
            Registry->OnDroneModelTypeChanged.AddDynamic(this, &UDroneListItemWidget::OnRegistryModelTypeChanged);
        }
    }
...
void UDroneListItemWidget::UpdateSelectionState()
{
    ...
    SelectButtonText->SetText(ProductText::Source(bCommandSelection
        ? (Registry->GetPrimarySelectedDrone() == DroneId ? TEXT("PRIMARY / 主选中") : (bSelected ? TEXT("MULTI / 多选中") : TEXT("选择无人机")))
        : (bSelected ? TEXT("取消选中") : TEXT("选中"))));
    if (bCommandSelection)
    {
        const bool bPrimary = Registry->GetPrimarySelectedDrone() == DroneId;
        CommandTheme::Button(SelectButton, bPrimary || bSelected);
        if (UBorder* Card = Cast<UBorder>(WidgetTree->RootWidget))
            CommandTheme::Panel(Card, bPrimary || bSelected ? CommandTheme::Selected : CommandTheme::Surface,
                bPrimary ? CommandTheme::Cyan : CommandTheme::Border);
    }
}
...
void UDroneListItemWidget::BuildRuntimeCard()
{
    if (WidgetTree)
    {
        ...
        // ---- 可折叠区域（DroneId行 → TargetIndexText，不含选中按钮） ----
        ...
        // ---- 无人机ID行（位于模式行上方） ----
        DroneIdText = MakeText(TEXT("DroneIdText"), TEXT("ID: --"), FLinearColor(0.55f, 0.62f, 0.70f, 1.0f));
        AddC(DroneIdText, FMargin(0.0f, 0.0f, 0.0f, 2.0f));

        // ---- 模式行 ----
        TaskRow = WidgetTree->ConstructWidget<UHorizontalBox>(UHorizontalBox::StaticClass(), TEXT("TaskRow"));
        ...
        AddC(TaskRow, FMargin(0.0f, 2.0f));

        // ---- 任务行（独立一行，紧跟模式行） ----
        UHorizontalBox* TaskStateRow = WidgetTree->ConstructWidget<UHorizontalBox>(UHorizontalBox::StaticClass(), TEXT("TaskStateRow"));
        TaskStateRow->AddChildToHorizontalBox(MakeText(TEXT("TaskLabel"), TEXT("任务  "), FLinearColor(0.48f, 0.67f, 0.84f, 1.0f)));
        TaskStatusText = MakeText(TEXT("TaskStatusText"), TEXT("待命"));
        if (UHorizontalBoxSlot* ChildSlot = TaskStateRow->AddChildToHorizontalBox(TaskStatusText)) ChildSlot->SetSize(FSlateChildSize(ESlateSizeRule::Fill));
        AddC(TaskStateRow, FMargin(0.0f, 2.0f));

        // ---- 外观模型行 ----
        UHorizontalBox* ModelRow = WidgetTree->ConstructWidget<UHorizontalBox>(UHorizontalBox::StaticClass(), TEXT("ModelRow"));
        ModelRow->AddChildToHorizontalBox(MakeText(TEXT("ModelLabel"), TEXT("外观  "), FLinearColor(0.48f, 0.67f, 0.84f, 1.0f)));
        ModelComboBox = WidgetTree->ConstructWidget<UComboBoxString>(UComboBoxString::StaticClass(), TEXT("ModelComboBox"));
        ModelComboBox->SetContentPadding(FMargin(8.0f, 3.0f));
        if (UHorizontalBoxSlot* ComboSlot = ModelRow->AddChildToHorizontalBox(ModelComboBox))
        {
        ...
        // StatusSourceText 附在独立一行，用 CollapsibleContent slot 的 widget 引用追踪
        // （折叠状态由 SetHideTaskRow 同步）

        GpsText = MakeText(TEXT("GpsText"), TEXT("经度: --\n纬度: --"));
        AddC(GpsText);
    ...
    auto MakeText = [this](UTextBlock*& Ptr, const FString& Label, FLinearColor Color = FLinearColor::White)
    {
        ...
        if (Ptr)
        {
            Ptr->SetText(ProductText::Source(Label));
...
void UDroneListItemWidget::ApplyCollapseState()
{
    ...
    if (CollapseButtonText)
    {
        CollapseButtonText->SetText(
            ProductText::Source(bIsCollapsed ? TEXT("▶") : TEXT("▼")));
    }
}
...
void UDroneListItemWidget::SetDroneFullState(int32 InDroneId, const FDroneTelemetrySnapshot& Snap)
{
    ...
    if (DroneIdText)
        DroneIdText->SetText(ProductText::Source(FString::Printf(TEXT("ID: %d"), InDroneId)));
    ...
    // 同时写入 ConnectionStatusText（新字段）和 StatusText（Blueprint 实际绑定的旧字段）
    {
        ...
        if (bCommandSelection)
        {
            Color = Snap.Availability == EDroneAvailability::Online ? CommandTheme::Online
                : (Snap.Availability == EDroneAvailability::Lost ? CommandTheme::Warning : CommandTheme::SecondaryText);
            ConnText = Snap.Availability == EDroneAvailability::Online ? TEXT("ONLINE / 在线")
                : (Snap.Availability == EDroneAvailability::Lost ? TEXT("LOST / 失联") : TEXT("OFFLINE / 离线"));
        }
        if (ConnectionStatusText)
        {
            ConnectionStatusText->SetText(ProductText::Source(ConnText));
            ConnectionStatusText->SetColorAndOpacity(Color);
        }
        if (StatusText)
        {
            StatusText->SetText(ProductText::Source(ConnText));
    ...
    if (GpsText)
    {
        GpsText->SetText(ProductText::Source(FString::Printf(
            TEXT("经度: %.6f\n纬度: %.6f"), Snap.GpsLongitude, Snap.GpsLatitude)));
    }
    ...
    if (AltitudeText)
    {
        AltitudeText->SetText(ProductText::Source(FString::Printf(
    ...
    if (BatteryText)
    {
        ...
        BatteryText->SetText(ProductText::Source(BatText));
    }
    ...
    if (TaskStatusText)
    {
        ...
        TaskStatusText->SetText(ProductText::Source(DisplayText));
    ...
    if (StatusSourceText)
    {
        ...
        StatusSourceText->SetText(ProductText::Source(
    ...
    if (WaypointText)
    {
        WaypointText->SetText(ProductText::Source(FString::Printf(
    ...
    if (TargetIndexText)
    {
        TargetIndexText->SetText(ProductText::Source(FString::Printf(
    ...
    if (UpdateTimeText)
    {
        ...
        UpdateTimeText->SetText(ProductText::Source(TimeStr));
    }
}
...
void UDroneListItemWidget::ApplyLabelSettings(const FDroneLabelSettings& Settings)
{
    if (DroneNameText)
    {
        DroneNameText->SetText(ProductText::Source(Settings.DisplayName));
        DroneNameText->SetColorAndOpacity(Settings.LabelColor);
    }
}

void UDroneListItemWidget::OnModelComboChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
    // SyncModelComboSelection() 触发的 Direct 回调不是用户改动，忽略以避免写回 Registry
    if (bSuppressModelComboCallback || DroneId <= 0)
    {
        return;
    }

    EDroneModelType NewType = EDroneModelType::Original;
    if (!TryGetDroneModelTypeByDisplayName(SelectedItem, NewType))
    {
        UE_LOG(LogTemp, Warning, TEXT("DroneListItemWidget: 未识别的模型选项 \"%s\""), *SelectedItem);
        return;
    }

    UGameInstance* GI = GetGameInstance();
    if (UDroneRegistrySubsystem* Registry = GI ? GI->GetSubsystem<UDroneRegistrySubsystem>() : nullptr)
    {
        Registry->SetDroneModelType(DroneId, NewType);
    }
}

void UDroneListItemWidget::OnRegistryModelTypeChanged(int32 InDroneId, EDroneModelType ModelType)
{
...
void UDroneListItemWidget::SyncModelComboSelection()
{
    if (!ModelComboBox || DroneId <= 0)
    {
        return;
    }
    ...
    const FString DisplayName = GetDroneModelDisplayName(Registry->GetDroneModelType(DroneId));

    bSuppressModelComboCallback = true;
    ModelComboBox->SetSelectedOption(DisplayName);
    bSuppressModelComboCallback = false;
}
