...
public:
    /** Command adapter entries: reuse the same editing and confirmation state machine. */
    bool BeginCommandPlanning();
    bool ConfirmCommandPath();
    bool CancelCommandPlanning();
    void SetCommandPresentation();
    void ToggleCommandTools();
    void SaveCommandPath() { OnSavePathClicked(); }
    void StopCommandPlayback() { OnGlobalStopClicked(); }
...
private:
    bool bCommandPresentation = false;
