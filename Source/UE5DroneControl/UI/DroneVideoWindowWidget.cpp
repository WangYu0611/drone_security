...
FReply UDroneVideoWindowWidget::NativeOnPreviewKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	if (ADroneOpsPlayerController* PlayerController = Cast<ADroneOpsPlayerController>(GetOwningPlayer()))
	{
		if (PlayerController->HandleVideoKeyboardForwardedKey(InKeyEvent.GetKey(), true))
		{
...
FReply UDroneVideoWindowWidget::NativeOnKeyUp(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	if (ADroneOpsPlayerController* PlayerController = Cast<ADroneOpsPlayerController>(GetOwningPlayer()))
	{
		if (PlayerController->HandleVideoKeyboardForwardedKey(InKeyEvent.GetKey(), false))
		{
			return FReply::Handled();
		}
	}
...
void UDroneVideoWindowWidget::NativeOnFocusLost(const FFocusEvent& InFocusEvent)
{
	if (ADroneOpsPlayerController* PlayerController = Cast<ADroneOpsPlayerController>(GetOwningPlayer()))
	{
		PlayerController->ClearVideoKeyboardForwardedKeys();
	}
