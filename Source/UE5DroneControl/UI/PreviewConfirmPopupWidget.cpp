...
void UPreviewConfirmPopupWidget::BuildRuntimeWidgetTree()
{
	...
	CommandTheme::Text(MessageText, 17, CommandTheme::PrimaryText, true);
	if (UVerticalBoxSlot* MessageSlot = Content->AddChildToVerticalBox(MessageText))
	{
		MessageSlot->SetPadding(FMargin(0.0f, 0.0f, 0.0f, 10.0f));
		MessageSlot->SetSize(FSlateChildSize(ESlateSizeRule::Automatic));
	}
	AttackPromptText = WidgetTree->ConstructWidget<UTextBlock>(UTextBlock::StaticClass(), TEXT("AttackPromptText"));
	AttackPromptText->SetText(FText::FromString(TEXT("是否攻击？")));
	AttackPromptText->SetJustification(ETextJustify::Center);
	AttackPromptText->SetColorAndOpacity(FLinearColor(0.94f, 0.97f, 1.0f, 1.0f));
	AttackPromptText->SetVisibility(ESlateVisibility::Collapsed);
	if (UVerticalBoxSlot* PromptSlot = Content->AddChildToVerticalBox(AttackPromptText))
	{
		PromptSlot->SetPadding(FMargin(0.0f, 0.0f, 0.0f, 18.0f));
	...
	auto MakeButton = [this, Content](const FName Name, const FString& Label, UButton*& OutButton, const FLinearColor& Color)
	{
		...
		CommandTheme::Button(OutButton);
		UTextBlock* Text = WidgetTree->ConstructWidget<UTextBlock>(UTextBlock::StaticClass());
		Text->SetText(FText::FromString(Label));
		Text->SetJustification(ETextJustify::Center);
		CommandTheme::Text(Text, 14);
	...
	const FVector& InTargetLocation)
{
	...
	if (AttackPromptText)
	{
		AttackPromptText->SetVisibility(ESlateVisibility::SelfHitTestInvisible);
	}
