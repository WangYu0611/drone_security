...
public:
	...
	/** True if this shadow drone represents an enemy target (DroneId >= DroneOpsConst::EnemyIdBase).
	 *  Set by DroneOpsGameMode at spawn time; causes BeginPlay to connect this actor as
	 *  the MovingRepresentative of the corresponding AHostileTargetActor. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Drone Identity")
	...
	void MoveToTarget3D(FVector WorldTarget, float RequestedSpeedMps = 0.0f);
	...
	/**
	 * 敌对无人机被我方攻击命中后置为 true。
	 * C++ 攻击完成时自动找到对应 DronePathActor 调用 PauseMovement() 停止路径驱动的移动。
	 * 此标志供外部（UI、Blueprint）查询用。
	 */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Drone|Attack")
	bool bIsAttackedAndStopped = false;
	...
	/**
	 * Like StopPatrolAndAttack but tracks a moving AHostileTargetActor every Tick.
	 * @param EnemyTargetId  AHostileTargetActor::TargetId of the enemy to pursue.
	 * @param InitialTargetLocation  Current location of the enemy at confirm time (used as first frame target).
	 */
	UFUNCTION(BlueprintCallable, Category = "Drone|Attack")
	void StartTrackingAttack(int32 EnemyTargetId, const FVector& InitialTargetLocation);
...
private:
	...
	/** Actual visual speed for the current one-shot map dispatch. 0 means use the configured fallback. */
	float ActiveOneShot3DMoveSpeedCmPerSec = 600.0f;
	...
	/** Swap the visual model when Registry broadcasts a per-drone model change. */
	UFUNCTION()
	...
	/**
	 * TargetId of the AHostileTargetActor being tracked (DroneOpsConst::DroneIdToTargetId applied).
	 * -1 means not tracking a moving enemy target; AttackTargetLocation is used as-is.
	 * When >= 0, Tick re-queries GetHostileTargetLocation() every frame to track movement.
	 */
	int32 AttackTargetEnemyId = -1;
