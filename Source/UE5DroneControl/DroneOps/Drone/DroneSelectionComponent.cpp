...
UDroneSelectionComponent::UDroneSelectionComponent()
{
	...
	// PrimarySelectedOverlayMaterial / HoveredOverlayMaterial 在蓝图 Details 面板中赋值，
	// 不在此处硬编码加载，避免关卡间资产流送差异导致材质为 nullptr（高光全黑）。
}
...
void UDroneSelectionComponent::RefreshHighlight()
{
	ApplyVisualState();
}
...
void UDroneSelectionComponent::ApplyVisualState()
{
	...
	for (UPrimitiveComponent* Primitive : PrimitiveComponents)
	{
		...
		{
			const bool bEnemy = DroneOpsConst::IsEnemyId(DroneId);
			UMaterialInterface* OverlayMaterial = nullptr;
			if (bIsPrimarySelected)
			{
				OverlayMaterial = (bEnemy && EnemySelectedOverlayMaterial)
					? EnemySelectedOverlayMaterial
					: PrimarySelectedOverlayMaterial;
			}
			else if (bIsHovered)
			{
				UMaterialInterface* Base = (bEnemy && EnemySelectedOverlayMaterial)
					? EnemySelectedOverlayMaterial
					: PrimarySelectedOverlayMaterial;
				OverlayMaterial = HoveredOverlayMaterial ? HoveredOverlayMaterial.Get() : Base;
			}
