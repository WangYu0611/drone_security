...
public:
	...
	/** Overlay material used when an enemy target (DroneId >= EnemyIdBase) is selected. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Selection|Highlight")
	TObjectPtr<UMaterialInterface> EnemySelectedOverlayMaterial = nullptr;
	...
	/**
	 * 重新应用当前选中/悬停视觉状态到 Owner 的全部图元组件。
	 * ApplyVisualState() 只在状态变化时被调用，因此运行时增删图元组件的场合
	 * （例如切换无人机模型后）需要显式调用一次，否则新组件拿不到 overlay 材质与描边。
	 */
	UFUNCTION(BlueprintCallable, Category = "Selection")
