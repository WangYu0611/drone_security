...
public:
	ADroneOpsGameMode();
    class UCommandMapInteractionService* GetCommandMapService() const { return CommandMapService; }
private:
    UPROPERTY(Transient) TObjectPtr<class UCommandMapInteractionService> CommandMapService;
public:
...
protected:
	...
	/** Dedicated spawn class for enemy shadow drones. Falls back to ShadowDroneClass when not set. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "DroneOps|Drone Spawn")
	TSubclassOf<AMultiDroneCharacter> EnemyShadowDroneClass;
