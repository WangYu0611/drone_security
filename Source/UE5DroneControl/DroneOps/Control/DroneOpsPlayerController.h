...
class UDefaultSegmentSpeedWidget;
...
public:
    ...
    class UCommandScreenManager* GetCommandScreenManager() const { return CommandScreenManager; }
    bool FocusCommandDrone(int32 DroneId);
    bool FocusCommandLocation(const FVector& WorldLocation);
    bool DeleteCommandWaypoint();
    void RefreshCommandSelection();
    bool IsCommandMapViewActive() const;

private:
    UPROPERTY(Transient) TObjectPtr<class UCommandScreenManager> CommandScreenManager;
public:
...
FGeographicDispatchResult AddGeographicWaypointsInEditMode(
	...
	/** Sets the speed for subsequently clicked waypoints; 0 is the unlimited-speed sentinel. */
	UFUNCTION(BlueprintCallable, Category = "DroneOps|PathEdit")
	void SetEditDefaultSegmentSpeed(float SpeedMps);

	UFUNCTION(BlueprintPure, Category = "DroneOps|PathEdit")
	float GetEditDefaultSegmentSpeed() const { return EditDefaultSegmentSpeed; }
	...
	/**
	 * 解析运行锚点无人机：多选时取编号最小的已选无人机，单选时取主选中无人机；
	 * 未选中时才回退到最小可用 DroneId。都没有则返回 false。
...
public:
	...
	/**
	 * Enter/leave the video keyboard-control mode. Entering requires exactly one
	 * selected drone; while active, map/path/queue interactions are disabled and
	 * WASD/QE drive only that locked drone.
	 */
	UFUNCTION(BlueprintCallable, Category = "DroneOps|VideoControl")
	bool SetVideoKeyboardControlEnabled(bool bEnabled);

	UFUNCTION(BlueprintCallable, Category = "DroneOps|VideoControl")
	bool ToggleVideoKeyboardControl();

	UFUNCTION(BlueprintPure, Category = "DroneOps|VideoControl")
	bool IsVideoKeyboardControlActive() const { return bVideoKeyboardControlActive; }

	UFUNCTION(BlueprintPure, Category = "DroneOps|VideoControl")
	int32 GetVideoKeyboardControlDroneId() const { return VideoKeyboardControlDroneId; }

	/** Called by an independent video SWindow so it can forward its focused keys. */
	bool HandleVideoKeyboardForwardedKey(const FKey& Key, bool bPressed);

	/** Clear all forwarded keys when the video window loses OS/Slate focus. */
	void ClearVideoKeyboardForwardedKeys();
...
bool ValidateDispatchDrone(int32 DroneId, FString& OutError) const;
	...
	// 等效"放大碰撞体"的抓取：把光标反投影成世界射线，取射线到航点球心的垂距
	// 在放大后半径内、且离相机最近的可交互航点。
	// 光标直接命中航点碰撞体时用不到它；它只负责"差一点点没点中"的情况，避免误加新航点。
	// 不真的放大 MeshComponent 的碰撞体：那会挡在 gizmo 轴命中体前面（GetHitResultUnderCursor
	// 取最近命中），把轴拖拽抢走。独立查询则完全不影响既有射线检测。
	ADroneWaypointActor* FindEditWaypointNearCursor() const;

	// 就近抓取命中后的选中处理（含 Shift/Ctrl 加选），只选中不进入拖拽。
	void SelectEditWaypointFromProximity(ADroneWaypointActor* WaypointActor);

	// Shift/Ctrl 任一按下即为加选修饰键。原先内联在 HandleEditModePressed 里，
	// 就近抓取也要用同一口径，提取出来避免两处漂移。
	bool IsAdditiveSelectionModifierDown() const;
	...
	// 航点点选的等效碰撞体放大倍数：判定半径 = 球体世界半径 × 本倍数。
	// 2.0 表示"当作一个两倍大的球来点"。倍数而非绝对距离，所以自动跟随
	// 关卡缩放（CesiumWorld 2.5×）与选中放大（1.5×），不必分别调参。
	UPROPERTY(EditAnywhere, Category = "PathEdit", meta = (ClampMin = "1.0", UIMin = "1.0"))
	float EditWaypointGrabRadiusMultiplier = 2.0f;

	// 在放大后的半径上再加的绝对余量（cm）。倍数对很小的球体给不出足够余量时兜底。
	UPROPERTY(EditAnywhere, Category = "PathEdit", meta = (ClampMin = "0.0", UIMin = "0.0"))
	float EditWaypointGrabExtraRadiusCm = 10.0f;
	...
	// ---- 视频流单机键盘控制 ----
	// This is deliberately controller-owned rather than pawn-owned: it locks the
	// current selection and keeps the persisted camera-control calibration per DroneId.
	bool bVideoKeyboardControlActive = false;
	int32 VideoKeyboardControlDroneId = INDEX_NONE;
	FVector VideoKeyboardTargetWorld = FVector::ZeroVector;
	float VideoKeyboardSendAccumulator = 0.0f;
	bool bVideoKeyboardMoveKeyWasDown = false;
	int32 VideoKeyboardRotationQuarterTurns = 0;
	bool VideoKeyboardHorizontalFlipped = false;
	TSet<FKey> VideoKeyboardForwardedKeys;

	/** Seconds between UE -> backend target updates while a key is held. */
	UPROPERTY(EditAnywhere, Category = "VideoControl")
	float VideoKeyboardCommandIntervalSeconds = 0.20f;

	void TickVideoKeyboardControl(float DeltaTime);
	void SendVideoKeyboardTarget();
	void LoadVideoKeyboardCalibration(int32 DroneId);
	void SaveVideoKeyboardCalibration() const;
	void RotateVideoKeyboardControlClockwise();
	void ToggleVideoKeyboardHorizontalFlip();
	FVector GetVideoKeyboardMoveDirection() const;
	bool IsVideoKeyboardKeyDown(const FKey& Key) const;

	UPROPERTY(EditDefaultsOnly, Category = "PathEdit|UI")
	TSubclassOf<UDefaultSegmentSpeedWidget> DefaultSegmentSpeedWidgetClass;

	UPROPERTY()
	TObjectPtr<UDefaultSegmentSpeedWidget> DefaultSegmentSpeedWidget;

	void ShowDefaultSegmentSpeedWidget();
	void HideDefaultSegmentSpeedWidget();
