...
#include "UI/GroundProjectionSettingsWidget.h"
...
namespace
{
	bool TryGetGeographicCoordinateAtWorldLocation(
		const UDroneRegistrySubsystem* Registry,
		...
		FGeographicCoordinate3D& OutCoordinate)
	{
		...
		const TScriptInterface<ICoordinateService> CoordinateService = Registry->GetCoordinateService();
		UObject* CoordinateServiceObject = CoordinateService.GetObject();
		if (!CoordinateServiceObject
			|| !ICoordinateService::Execute_IsCoordinateSystemReady(CoordinateServiceObject)
			|| !ICoordinateService::Execute_IsGeographicSupported(CoordinateServiceObject))
		{
			return false;
		}

		const FVector LongitudeLatitudeHeight =
			ICoordinateService::Execute_WorldToGeographic(CoordinateServiceObject, WorldLocation);
		if (!FMath::IsFinite(LongitudeLatitudeHeight.X)
			|| LongitudeLatitudeHeight.X < -180.0 || LongitudeLatitudeHeight.X > 180.0
			|| !FMath::IsFinite(LongitudeLatitudeHeight.Y)
			|| LongitudeLatitudeHeight.Y < -90.0 || LongitudeLatitudeHeight.Y > 90.0
			|| !FMath::IsFinite(LongitudeLatitudeHeight.Z))
		{
			return false;
		}

		OutCoordinate.Longitude = LongitudeLatitudeHeight.X;
		OutCoordinate.Latitude = LongitudeLatitudeHeight.Y;
		OutCoordinate.AltitudeMslMeters = LongitudeLatitudeHeight.Z;
		return true;
	}

	bool TryGetMapNorthYawDegrees(
		const UDroneRegistrySubsystem* Registry,
		const FVector& WorldLocation,
		float& OutYawDegrees)
	{
		FGeographicCoordinate3D Coordinate;
		if (!TryGetGeographicCoordinateAtWorldLocation(Registry, WorldLocation, Coordinate))
		{
			return false;
		}
		...
		const TScriptInterface<ICoordinateService> CoordinateService = Registry->GetCoordinateService();
		UObject* CoordinateServiceObject = CoordinateService.GetObject();
		check(CoordinateServiceObject);

		// A short northward geographic offset gives the actual local north vector in
		// the current Cesium world, regardless of whether the visible map is online
		// tiles or the offline raster plane.
		constexpr double NorthSampleMeters = 10.0;
		constexpr double DegreesPerMeter = 1.0 / 111320.0;
		const FVector NorthWorld = ICoordinateService::Execute_GeographicToWorld(
			CoordinateServiceObject,
			Coordinate.Latitude + NorthSampleMeters * DegreesPerMeter,
			Coordinate.Longitude,
			Coordinate.AltitudeMslMeters);
		const FVector NorthDirection = FVector(NorthWorld.X - WorldLocation.X, NorthWorld.Y - WorldLocation.Y, 0.0).GetSafeNormal();
		if (NorthDirection.IsNearlyZero())
		{
			return false;
		}

		OutYawDegrees = NorthDirection.Rotation().Yaw;
		return FMath::IsFinite(OutYawDegrees);
	}

	bool IsAnyDroneOpsPanelInteractive()
	{
		return USequenceDispatchPanelWidget::IsPanelInteractive()
			|| UGeographicTargetPanelWidget::IsPanelInteractive()
			|| UGroundProjectionSettingsWidget::IsPanelInteractive();
	}
...
void ADroneOpsPlayerController::BeginPlay()
{
    ...
    {
        DroneRegistry = GetGameInstance()->GetSubsystem<UDroneRegistrySubsystem>();
        CommandScreenManager = NewObject<UCommandScreenManager>(this);
        CommandScreenManager->Initialize(this);
        if (UCommandScreenManager::ResolveClientRole() == EDroneClientRole::Command && GetWorld()->GetGameViewport())
            GetWorld()->GetGameViewport()->bDisableWorldRendering = true;
        SetInputMode(FInputModeUIOnly());
        bShowMouseCursor = true;
        SetActorTickEnabled(false);
        return; // No legacy HUD, tools, info panels or scene-control setup in Video.
    }

	if (GEngine && UCommandScreenManager::ResolveClientRole() == EDroneClientRole::Standalone)
	{
	...
	if (DroneOpsHUDWidgetClass && IsLocalController() && UCommandScreenManager::ResolveClientRole() == EDroneClientRole::Standalone)
	{
	...
	// Compatibility entry points now delegate Command panels to the shell.
	...
	UUIManagerBlueprintLibrary::ShowEnemyDroneList(this);
	UUIManagerBlueprintLibrary::LinkPanels();

	// 默认航段速度属于预演阶段的全局输入，不必等到开启“路径编辑”才显示。
	// 这样在 CesiumWorld 中始终能看到并预先设置它；数值只会在之后点击
	// 新航点时被读取，不会回写既有航段。
    if (UCommandScreenManager::ResolveClientRole() == EDroneClientRole::Standalone) ShowDefaultSegmentSpeedWidget();
...
void ADroneOpsPlayerController::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    ...
    if (DroneRegistry) DroneRegistry->OnMultiSelectionChanged.RemoveDynamic(this, &ADroneOpsPlayerController::OnRegistryMultiSelectionChanged);
    if (CommandScreenManager) { CommandScreenManager->Destroy(); CommandScreenManager = nullptr; }
...
void ADroneOpsPlayerController::SetupInputComponent()
{
    ...
    if (UCommandScreenManager::ResolveClientRole() == EDroneClientRole::Video || UCommandScreenManager::ResolveClientRole() == EDroneClientRole::Command) return;
	...
	if (InputComponent)
	{
		...
		InputComponent->BindKey(EKeys::R, IE_Pressed, this, &ADroneOpsPlayerController::RotateVideoKeyboardControlClockwise);
...
void ADroneOpsPlayerController::Tick(float DeltaTime)
{
	...
	// editing, and map click dispatch from competing with the live video feed.
	if (bVideoKeyboardControlActive)
	{
		TickVideoKeyboardControl(DeltaTime);
		return;
	}

    const bool bCommandView = IsCommandMapViewActive();
    if (bCommandView) CommandScreenManager->GetMapService()->TickView(DeltaTime);
	...
	if (bPathEditMode)
	{
		...
		{
			// 拖拽航点时不移动相机，避免冲突
			if (!bCommandView) TickEditCamera(DeltaTime);
		}
		return;
	}
...
bool ADroneOpsPlayerController::ToggleVideoKeyboardControl()
{
	return SetVideoKeyboardControlEnabled(!bVideoKeyboardControlActive);
}

bool ADroneOpsPlayerController::SetVideoKeyboardControlEnabled(bool bEnabled)
{
	if (!bEnabled)
	{
		bVideoKeyboardControlActive = false;
		VideoKeyboardControlDroneId = INDEX_NONE;
		ClearVideoKeyboardForwardedKeys();
		return true;
	}
	...
	{
		UUIManagerBlueprintLibrary::ShowToast(this, TEXT("视频键盘操控只能选择一架无人机"), 3.0f);
		return false;
	}
	if (bPathEditMode)
	{
		UUIManagerBlueprintLibrary::ShowToast(this, TEXT("请先退出路径点编辑，再进入视频键盘操控"), 3.0f);
		return false;
	}

	FString ValidationError;
	const int32 DroneId = SelectedIds[0];
	if (!ValidateDispatchDrone(DroneId, ValidationError))
	{
		UUIManagerBlueprintLibrary::ShowToast(this, ValidationError, 3.0f);
		return false;
	}

	AActor* DroneActor = ResolveDroneActorById(DroneId);
	if (!IsValid(DroneActor))
	{
		UUIManagerBlueprintLibrary::ShowToast(this, TEXT("选中的无人机不可用"), 3.0f);
		return false;
	}

	StopSelectedDroneVerticalControl(false);
	bVideoKeyboardControlActive = true;
	VideoKeyboardControlDroneId = DroneId;
	VideoKeyboardTargetWorld = DroneActor->GetActorLocation();
	ClearVideoKeyboardForwardedKeys();
	LoadVideoKeyboardCalibration(DroneId);
	UUIManagerBlueprintLibrary::ShowToast(this, TEXT("已进入视频键盘操控：WASD 移动，Q/E 升降，R 转向，F 翻转左右"), 4.0f);
	return true;
}

void ADroneOpsPlayerController::LoadVideoKeyboardCalibration(int32 DroneId)
{
	VideoKeyboardRotationQuarterTurns = 0;
	VideoKeyboardHorizontalFlipped = false;
	if (!GConfig || DroneId <= 0) return;

	const FString Section = FString::Printf(TEXT("VideoKeyboardControl.Drone.%d"), DroneId);
	GConfig->GetInt(*Section, TEXT("RotationQuarterTurns"), VideoKeyboardRotationQuarterTurns, GGameUserSettingsIni);
	GConfig->GetBool(*Section, TEXT("HorizontalFlipped"), VideoKeyboardHorizontalFlipped, GGameUserSettingsIni);
	VideoKeyboardRotationQuarterTurns = (VideoKeyboardRotationQuarterTurns % 4 + 4) % 4;
}

void ADroneOpsPlayerController::SaveVideoKeyboardCalibration() const
{
	if (!GConfig || VideoKeyboardControlDroneId <= 0) return;

	const FString Section = FString::Printf(TEXT("VideoKeyboardControl.Drone.%d"), VideoKeyboardControlDroneId);
	GConfig->SetInt(*Section, TEXT("RotationQuarterTurns"), VideoKeyboardRotationQuarterTurns, GGameUserSettingsIni);
	GConfig->SetBool(*Section, TEXT("HorizontalFlipped"), VideoKeyboardHorizontalFlipped, GGameUserSettingsIni);
	GConfig->Flush(false, GGameUserSettingsIni);
}

void ADroneOpsPlayerController::RotateVideoKeyboardControlClockwise()
{
	if (!bVideoKeyboardControlActive)
	{
		ResetShadowDronesToMirrors();
		return;
	}

	VideoKeyboardRotationQuarterTurns = (VideoKeyboardRotationQuarterTurns + 1) % 4;
	SaveVideoKeyboardCalibration();
	UUIManagerBlueprintLibrary::ShowToast(this,
		FString::Printf(TEXT("视频操控方向已顺时针旋转 %d°"), VideoKeyboardRotationQuarterTurns * 90), 2.0f);
}

void ADroneOpsPlayerController::ToggleVideoKeyboardHorizontalFlip()
{
	if (!bVideoKeyboardControlActive) return;
	VideoKeyboardHorizontalFlipped = !VideoKeyboardHorizontalFlipped;
	SaveVideoKeyboardCalibration();
	UUIManagerBlueprintLibrary::ShowToast(this,
		VideoKeyboardHorizontalFlipped ? TEXT("视频操控：A/D 已左右翻转") : TEXT("视频操控：A/D 已恢复默认"), 2.0f);
}

bool ADroneOpsPlayerController::HandleVideoKeyboardForwardedKey(const FKey& Key, bool bPressed)
{
	if (!bVideoKeyboardControlActive)
	{
		return false;
	...
	const bool bIsVideoKeyboardControlKey =
		Key == EKeys::W || Key == EKeys::A || Key == EKeys::S || Key == EKeys::D ||
		Key == EKeys::Q || Key == EKeys::E || Key == EKeys::R || Key == EKeys::F;
	if (!bIsVideoKeyboardControlKey)
	{
		return false;
	}

	const bool bWasDown = VideoKeyboardForwardedKeys.Contains(Key);
	if (bPressed)
	{
		VideoKeyboardForwardedKeys.Add(Key);
		if (!bWasDown)
		{
			if (Key == EKeys::R)
			{
				RotateVideoKeyboardControlClockwise();
			}
			else if (Key == EKeys::F)
			{
				ToggleVideoKeyboardHorizontalFlip();
			}
		}
	}
	else
	{
		VideoKeyboardForwardedKeys.Remove(Key);
		if (VideoKeyboardForwardedKeys.Num() == 0)
		{
			VideoKeyboardSendAccumulator = 0.0f;
			bVideoKeyboardMoveKeyWasDown = false;
		}
	}
...
void ADroneOpsPlayerController::ClearVideoKeyboardForwardedKeys()
{
	VideoKeyboardForwardedKeys.Reset();
	VideoKeyboardSendAccumulator = 0.0f;
	bVideoKeyboardMoveKeyWasDown = false;
}

bool ADroneOpsPlayerController::IsVideoKeyboardKeyDown(const FKey& Key) const
{
	return VideoKeyboardForwardedKeys.Contains(Key) || IsInputKeyDown(Key);
}

FVector ADroneOpsPlayerController::GetVideoKeyboardMoveDirection() const
{
	FVector Direction = FVector::ZeroVector;
	if (IsVideoKeyboardKeyDown(EKeys::W)) Direction.X += 1.0f;
	if (IsVideoKeyboardKeyDown(EKeys::S)) Direction.X -= 1.0f;
	const float Lateral = (IsVideoKeyboardKeyDown(EKeys::D) ? 1.0f : 0.0f) - (IsVideoKeyboardKeyDown(EKeys::A) ? 1.0f : 0.0f);
	Direction.Y = VideoKeyboardHorizontalFlipped ? -Lateral : Lateral;

	for (int32 Index = 0; Index < VideoKeyboardRotationQuarterTurns; ++Index)
	{
		// +90 degrees in UE XY: W becomes right, then back, then left.
		Direction = FVector(-Direction.Y, Direction.X, Direction.Z);
	}
	if (IsVideoKeyboardKeyDown(EKeys::E)) Direction.Z += 1.0f;
	if (IsVideoKeyboardKeyDown(EKeys::Q)) Direction.Z -= 1.0f;
	return Direction.GetSafeNormal();
}

void ADroneOpsPlayerController::TickVideoKeyboardControl(float DeltaTime)
{
	const TArray<int32> SelectedIds = GetSelectedDroneIdsForDispatch();
	if (!DroneRegistry || SelectedIds.Num() != 1
		|| !SelectedIds.Contains(VideoKeyboardControlDroneId))
	{
		SetVideoKeyboardControlEnabled(false);
		UUIManagerBlueprintLibrary::ShowToast(this, TEXT("选中无人机已改变，已退出视频键盘操控"), 3.0f);
		return;
	}

	const FVector Direction = GetVideoKeyboardMoveDirection();
	if (Direction.IsNearlyZero())
	{
		// Releasing a key immediately stops accumulating targets. The latest target
		// remains valid, so the aircraft completes only the already-merged segment.
		VideoKeyboardSendAccumulator = 0.0f;
		bVideoKeyboardMoveKeyWasDown = false;
		return;
	}

	VideoKeyboardTargetWorld += Direction * FMath::Max(EditDefaultSegmentSpeed, 0.5f) * 100.0f * DeltaTime;
	if (AMultiDroneCharacter* ShadowDrone = Cast<AMultiDroneCharacter>(ResolveDroneActorById(VideoKeyboardControlDroneId)))
	{
		// The UE shadow is visual-only. Update its target every frame so it tracks
		// the held key smoothly, while SendVideoKeyboardTarget keeps real-flight
		// command dispatch rate-limited to the configured 0.2-second interval.
		ShadowDrone->MoveToTarget3D(VideoKeyboardTargetWorld, 0.0f);
	}

	if (!bVideoKeyboardMoveKeyWasDown)
	{
		// First press has no debounce delay. Further targets are rate-limited below.
		bVideoKeyboardMoveKeyWasDown = true;
		VideoKeyboardSendAccumulator = 0.0f;
		SendVideoKeyboardTarget();
		return;
	}
	VideoKeyboardSendAccumulator += DeltaTime;
	if (VideoKeyboardSendAccumulator >= FMath::Max(VideoKeyboardCommandIntervalSeconds, 0.05f))
	{
		VideoKeyboardSendAccumulator = 0.0f;
		SendVideoKeyboardTarget();
	}
...
void ADroneOpsPlayerController::SendVideoKeyboardTarget()
{
	if (!bVideoKeyboardControlActive || !DroneRegistry) return;

	UDroneNetworkManager* NetworkManager = GetGameInstance()
		? GetGameInstance()->GetSubsystem<UDroneNetworkManager>() : nullptr;
	if (!NetworkManager || !NetworkManager->CanSendToBackend()
		|| !NetworkManager->GetWebSocketClient() || !NetworkManager->GetWebSocketClient()->IsConnected()) return;

	ARealTimeDroneReceiver* Receiver = Cast<ARealTimeDroneReceiver>(DroneRegistry->GetReceiverActor(VideoKeyboardControlDroneId));
	if (!Receiver || !Receiver->bHasGpsAnchor)
	{
		UE_LOG(LogTemp, Warning, TEXT("Video keyboard control: drone %d has no GPS anchor"), VideoKeyboardControlDroneId);
		return;
	}

	NetworkManager->SendMoveCommand(
		VideoKeyboardControlDroneId,
		VideoKeyboardTargetWorld - Receiver->AnchorWorldLocation,
		DroneRegistry->GetDroneCommandMode(VideoKeyboardControlDroneId),
		false,
		// The backend recognizes the manual-target flag and serializes the Jetson
		// speed field as the explicit NaN marker for this live-control mode.
		0.0f,
		true);
}
...
void ADroneOpsPlayerController::OnPrimaryClick()
{
	...
	if (bVideoKeyboardControlActive)
	{
		// The controlled drone is locked for the duration of this mode. Do not let
		// a map click silently retarget or change the selected aircraft.
		return;
	}
	...
	// 面板处于交互状态时不处理游戏点击
	if (IsAnyDroneOpsPanelInteractive() && !IsCommandMapViewActive())
	{
		return;
	}
	...
	if (GEngine && !IsCommandMapViewActive())
	{
		if (DroneActor)
		{
...
void ADroneOpsPlayerController::FinishBoxSelectIfPending()
{
	...
	if (USequenceDispatchPanelWidget::IsPanelInteractive() && !IsCommandMapViewActive())
	{
		return;
	}
...
{
    if (IsCommandMapViewActive()) return; // Middle drag belongs to map pan; details live in the shell.
...
{
    if (CommandScreenManager && CommandScreenManager->GetClientRole() == EDroneClientRole::Command)
    {
        if (DroneRegistry && DroneRegistry->IsDroneRegistered(DroneId))
        {
            DroneRegistry->SetPrimarySelectedDrone(DroneId);
            DroneRegistry->SetMultiSelectedDrones({DroneId});
        }
        return; // Command already projects the selected drone's details.
    }
...
{
    if (UCommandScreenManager::ResolveClientRole() == EDroneClientRole::Video)
    {
        if (DroneRegistry) DroneRegistry->SetPrimarySelectedDrone(DroneId);
        if (CommandScreenManager) CommandScreenManager->Show();
        return CommandScreenManager && CommandScreenManager->GetVideoShell();
    }
	if (DroneId <= 0)
	{
		return false;
	...
	if (DroneRegistry)
	{
		...
		{
			if (Descriptor.bIsEnemyTarget)
			{
				UUIManagerBlueprintLibrary::ShowToast(this, TEXT("敌对无人机不支持视频流"), 3.0f);
				return false;
			}
...
{
    if (IsCommandMapViewActive()) { CommandScreenManager->GetMapService()->Enter3DMode(); return; }
	if (bVideoKeyboardControlActive)
	{
		ToggleVideoKeyboardHorizontalFlip();
		return;
...
{
	bShiftHeld = false;
    if (IsCommandMapViewActive()) return;
	...
	TArray<FGeographicDispatchSlot>& OutSlots) const
{
	...
	for (int32 Index = 0; Index < OrderedIds.Num(); ++Index)
	{
		...
		if (!ValidateDispatchDrone(DroneId, ValidateError))
		{
			// 敌对目标 ID（已删除或影子机不存在）直接跳过，不阻塞友方派发
			if (DroneOpsConst::IsEnemyId(DroneId))
			{
				UE_LOG(LogTemp, Warning, TEXT("BuildWorldDispatchPlan: skip enemy drone %d (%s)"), DroneId, *ValidateError);
				continue;
			}
		...
		Slot.bIsPrimary = SlotIndex == 0;
		Slot.WorldTarget = BaseWorldTarget + ComputeMultiDispatchOffset(SlotIndex, SpacingCm);
		OutSlots.Add(Slot);
		++SlotIndex;
	}
	...
	const TArray<FGeographicDispatchSlot>& Slots)
{
	...
	int32 BackendTargetConversionFailureCount = 0;
	for (const FGeographicDispatchSlot& Slot : Slots)
	{
		if (AMultiDroneCharacter* Shadow = Cast<AMultiDroneCharacter>(ResolveDroneActorById(Slot.DroneId)))
		{
			...
			Shadow->MoveToTarget3D(Slot.WorldTarget, EditDefaultSegmentSpeed);
			++LocalMoveCount;
		}
		...
		FVector SendLocation = Slot.WorldTarget - Receiver->AnchorWorldLocation;
		FGeographicCoordinate3D MapTargetCoordinate;
		if (TryGetGeographicCoordinateAtWorldLocation(DroneRegistry, Slot.WorldTarget, MapTargetCoordinate))
		{
			// Cesium world axes are not guaranteed to be UE +X=North/+Y=East. Convert
			// the clicked world target back to WGS84 first, then reuse the geographic
			// dispatch path to build a PX4-local NED target. This is shared by online
			// Cesium and the offline raster plane because both use the same georeference.
			MapTargetCoordinate.AltitudeMslMeters -= NetworkManager->GeoidSeparationMeters;
			FString TargetError;
			if (!TryBuildGeographicBackendTarget(
				...
				TargetError))
			{
				++BackendTargetConversionFailureCount;
				UE_LOG(LogTemp, Warning,
					TEXT("Map dispatch: skipped backend command for drone %d because Cesium world -> NED conversion failed: %s"),
					Slot.DroneId,
					*TargetError);
				continue;
			}
		}
		else if (DroneRegistry->GetCoordinateService()
			&& ICoordinateService::Execute_IsGeographicSupported(DroneRegistry->GetCoordinateService().GetObject()))
		{
			// Do not fall back to raw UE X/Y when a Cesium service is active but cannot
			// transform this point: that fallback is precisely what rotates north/east.
			++BackendTargetConversionFailureCount;
			UE_LOG(LogTemp, Warning,
				TEXT("Map dispatch: skipped backend command for drone %d because the Cesium coordinate service could not convert the clicked point"),
				Slot.DroneId);
			continue;
		}
	...
	else if (MissingAnchorCount > 0 || BackendTargetConversionFailureCount > 0)
	{
		Result.Message = FString::Printf(TEXT("已在UE内派发 %d 架；已发送后端 %d 架，%d 架等待GPS锚点，%d 架坐标转换失败"),
			LocalMoveCount, BackendSendCount, MissingAnchorCount, BackendTargetConversionFailureCount);
	}
	else
	{
...
bool ADroneOpsPlayerController::FocusCommandDrone(int32 DroneId)
{
    if (IsCommandMapViewActive())
    {
        const AActor* Drone = ResolveFollowViewTargetByDroneId(DroneId);
        return Drone && CommandScreenManager->GetMapService()->FocusLocation(Drone->GetActorLocation());
    }
    if (bPathEditMode || bVideoKeyboardControlActive) return false;
    if (!IsValid(ResolveFollowViewTargetByDroneId(DroneId))) return false;
    if (CameraModeState.CameraMode == EDroneCameraMode::Free) OnFreeCamToggle();
    CameraModeState.CameraMode = EDroneCameraMode::Follow;
    ApplyFollowViewTarget(DroneId);
    return true;
}
bool ADroneOpsPlayerController::FocusCommandLocation(const FVector& Location)
{
    if (IsCommandMapViewActive()) return CommandScreenManager->GetMapService()->FocusLocation(Location);
    if (Location.ContainsNaN() || bPathEditMode || bVideoKeyboardControlActive || !IsValid(FreeCamActor)) return false;
    FreeCamActor->SetActorLocation(Location + FVector(-3000, 0, 3000));
    FreeCamActor->SetActorRotation(FRotator(-45, 0, 0));
    FreeCamRotation = FreeCamActor->GetActorRotation();
    CameraModeState.CameraMode = EDroneCameraMode::Free;
    SetViewTargetWithBlend(FreeCamActor, 0.35f);
    return true;
}
bool ADroneOpsPlayerController::DeleteCommandWaypoint()
{
    if (!bPathEditMode) return false;
    int32 BeforeCount = 0;
    for (const auto& Pair : BuildEditingPathsData()) BeforeCount += Pair.Value.Waypoints.Num();
    HandleDeleteWaypointKey();
    int32 AfterCount = 0;
    for (const auto& Pair : BuildEditingPathsData()) AfterCount += Pair.Value.Waypoints.Num();
    return AfterCount < BeforeCount;
}
void ADroneOpsPlayerController::RefreshCommandSelection() { OnRegistryMultiSelectionChanged(); }
bool ADroneOpsPlayerController::IsCommandMapViewActive() const
{
    return CommandScreenManager && CommandScreenManager->GetClientRole() == EDroneClientRole::Map
        && CommandScreenManager->GetMapService() && CommandScreenManager->GetMapService()->OwnsCamera();
}
...
{
    if (IsCommandMapViewActive()) { FocusCommandDrone(DroneId); return; }
...
void ADroneOpsPlayerController::OnTopDownToggle()
{
    if (IsCommandMapViewActive())
    {
        auto* Map = CommandScreenManager->GetMapService();
        Map->SetMapMode(Map->GetMapMode() == ECommandMapMode::Map2D ? ECommandMapMode::Map3D : ECommandMapMode::Map2D);
        return;
    }
	...
	if (TryGetMapNorthYawDegrees(DroneRegistry, FocusLoc, NorthYawDegrees))
	{
		UE_LOG(LogTemp, Log, TEXT("TopDown: aligned map camera to true north (Yaw=%.2f)"), NorthYawDegrees);
	}
	TopDownCamActor->SetActorLocationAndRotation(
		FVector(FocusLoc.X, FocusLoc.Y, FocusLoc.Z + TopDownHeightCm),
		FRotator(-90.0f, NorthYawDegrees, 0.0f));
...
void ADroneOpsPlayerController::SwitchToNextMultiDroneFollowView(float BlendTime)
{
    if (IsCommandMapViewActive())
    {
        if (DroneRegistry) FocusCommandDrone(DroneRegistry->GetPrimarySelectedDrone());
        return;
    }
	UWorld* World = GetWorld();
...
void ADroneOpsPlayerController::RemoveHostileTarget(AHostileTargetActor* Target)
{
	...
	// 清理注册表中对应的敌对 drone 记录及影子机
	if (UDroneRegistrySubsystem* Registry = GetGameInstance()->GetSubsystem<UDroneRegistrySubsystem>())
	{
		const int32 DroneId = DroneOpsConst::TargetIdToDroneId(Target->TargetId);
		if (APawn* ShadowPawn = Registry->GetSenderPawn(DroneId))
		{
			ShadowPawn->Destroy();
		}
...
void ADroneOpsPlayerController::OnAttackConfirmMade(int32 DroneId, int32 TargetId, bool bAttack)
{
	...
	if (bAttack)
	{
		...
		// 停止巡逻并追踪移动目标（每 Tick 刷新目标位置）
		Shadow->StartTrackingAttack(TargetId, Target->GetHostileTargetLocation());
	}
	else
	{
...
bool ADroneOpsPlayerController::BeginPathEditMode(const TArray<int32>& DroneIds)
{
	if (bVideoKeyboardControlActive)
	{
		UUIManagerBlueprintLibrary::ShowToast(this, TEXT("视频键盘操控期间不支持路径点编辑"), 3.0f);
		return false;
	}
	if (!DroneRegistry)
	{
	...
	for (int32 DroneId : DroneIds)
	{
		...
		if (DroneOpsConst::IsEnemyId(DroneId))
		{
			// Enemy targets have no shadow pawn; use the registered initial location.
			FDroneDescriptor EnemyDesc;
			if (!DroneRegistry->GetDroneDescriptor(DroneId, EnemyDesc))
			{
				continue;
			}
			StartLocation = EnemyDesc.EnemyInitialLocation;
			// If no initial location was stored, fall back to a point in front of the camera.
			if (StartLocation.IsNearlyZero())
			{
				if (APlayerCameraManager* CamMgr = PlayerCameraManager)
				{
					StartLocation = CamMgr->GetCameraLocation()
						+ CamMgr->GetCameraRotation().Vector() * 10000.0f;
				}
			}
		}
		else
		{
			...
			StartLocation = ShadowPawn->GetActorLocation();
		}
...
void ADroneOpsPlayerController::SetEditDefaultSegmentSpeed(float SpeedMps)
{
	// Keep 0 as a JSON-safe unlimited-speed marker. The future Jetson packet
	// serializer will convert it to NaN at the protocol boundary.
	EditDefaultSegmentSpeed = FMath::IsNearlyZero(SpeedMps)
		? 0.0f
		: FMath::Clamp(SpeedMps, 0.5f, 15.0f);
}

void ADroneOpsPlayerController::ShowDefaultSegmentSpeedWidget()
{
    if (IsCommandMapViewActive()) return; // Available on demand through CommandScreenManager, never a second top-level overlay.
	if (IsValid(DefaultSegmentSpeedWidget))
	{
		DefaultSegmentSpeedWidget->SetDisplayedSpeed(EditDefaultSegmentSpeed);
		DefaultSegmentSpeedWidget->ShowOverlay();
		return;
	}

	TSubclassOf<UDefaultSegmentSpeedWidget> WidgetClass = DefaultSegmentSpeedWidgetClass;
	if (!WidgetClass)
	{
		WidgetClass = UDefaultSegmentSpeedWidget::StaticClass();
	}

	DefaultSegmentSpeedWidget = CreateWidget<UDefaultSegmentSpeedWidget>(this, WidgetClass);
	if (!IsValid(DefaultSegmentSpeedWidget))
	{
		UE_LOG(LogTemp, Warning, TEXT("[PathEdit] Failed to create default segment speed widget"));
		return;
	}

	DefaultSegmentSpeedWidget->SetPathEditController(this);
	DefaultSegmentSpeedWidget->SetDisplayedSpeed(EditDefaultSegmentSpeed);
	// 主 HUD 的层级是 50；放在其上方，保证 CesiumWorld 的全屏 HUD
	// 不会遮住输入框，也能正常点击。
	DefaultSegmentSpeedWidget->AddToViewport(60);
}

void ADroneOpsPlayerController::HideDefaultSegmentSpeedWidget()
{
	if (IsValid(DefaultSegmentSpeedWidget))
	{
		DefaultSegmentSpeedWidget->RemoveFromParent();
		DefaultSegmentSpeedWidget = nullptr;
	}
}

bool ADroneOpsPlayerController::IsAdditiveSelectionModifierDown() const
{
...
void ADroneOpsPlayerController::SelectEditWaypointFromProximity(ADroneWaypointActor* WaypointActor)
{
	...
	if (IsAdditiveSelectionModifierDown())
	{
		ToggleEditWaypointSelection(WaypointActor);
		return;
	}

	// 就近抓取只做选中，不进入拖拽：光标并未真的落在某条 gizmo 轴上，
	// 贸然开拖会让航点顺着一条用户没瞄准的轴跑掉。选中后再拖轴即可。
	SetEditSelectedWaypoint(WaypointActor);
...
void ADroneOpsPlayerController::HandleEditModePressed()
{
	...
	if (bMissionPathMode && CommandScreenManager && CommandScreenManager->GetMapService()
		&& CommandScreenManager->GetMapService()->GetMapMode() == ECommandMapMode::Map2D
		&& CanInteractWithEditWaypoint(EditSelectedWaypoint) && !IsAdditiveSelectionModifierDown())
	{
		double MouseX = 0, MouseY = 0;
		if (GetMousePosition(MouseX, MouseY))
		{
			const FVector2D Mouse(MouseX, MouseY);
			const TPair<EGizmoAxis,UArrowComponent*> Axes[] = {
				{EGizmoAxis::X,EditSelectedWaypoint->GizmoXAxisHandle},
				{EGizmoAxis::Y,EditSelectedWaypoint->GizmoYAxisHandle}};
			for (const auto& Axis : Axes)
			{
				if (!IsValid(Axis.Value)) continue;
				FVector2D Start, End;
				const FVector Origin = Axis.Value->GetComponentLocation();
				const FVector Tip = Origin + Axis.Value->GetForwardVector() * Axis.Value->ArrowLength * Axis.Value->GetComponentScale().X;
				if (!ProjectWorldLocationToScreen(Origin,Start) || !ProjectWorldLocationToScreen(Tip,End)) continue;
				const FVector2D Segment = End-Start;
				if (Segment.SizeSquared()<144) continue;
				const double T = FMath::Clamp(FVector2D::DotProduct(Mouse-Start,Segment)/Segment.SizeSquared(),0.0,1.0);
				if (FVector2D::DistSquared(Mouse,Start+Segment*T)>64) continue;
				SetEditActiveAxis(Axis.Key);bEditDraggingWaypoint=true;
				SetIgnoreLookInput(true);SetIgnoreMoveInput(true);
				CaptureMoveUndoStart();BeginDeferredUpdateForSelection();EditLastMouseScreenPos=Mouse;
				return;
			}
		}
	}
	...
	if (!GetHitResultUnderCursor(ECC_Visibility, false, HitResult))
	{
		// 没命中任何几何体（例如点到天空）：仍允许就近抓取，否则天际线前的航点选不中。
		if (ADroneWaypointActor* NearWaypoint = FindEditWaypointNearCursor())
		{
			SelectEditWaypointFromProximity(NearWaypoint);
			return;
		}
		SetEditSelectedWaypoint(nullptr);
		return;
	...
	{
		const bool bAdditiveModifier = IsAdditiveSelectionModifierDown();
	...
	// 命中地面/地形，且光标附近没有可抓取的航点 → 给所有临时路径追加航点（编队平移）。
	// 先做就近抓取：想选航点却差几个像素时，宁可选中它，也不要静默新建一个航点——
	// 新建是破坏性的（改变路径形状），选错则一眼可见、代价低。
	if (ADroneWaypointActor* NearWaypoint = FindEditWaypointNearCursor())
	{
		SelectEditWaypointFromProximity(NearWaypoint);
		return;
...
bool ADroneOpsPlayerController::CanInteractWithEditWaypoint(const ADroneWaypointActor* WaypointActor) const
{
	...
	if (WaypointActor->WaypointIndex == 0 && !bMissionPathMode)
	{
		return false;
	}
...
ADroneWaypointActor* ADroneOpsPlayerController::FindEditWaypointNearCursor() const
{
	if (!bPathEditMode)
	{
		return nullptr;
	...
	// 等效于"把航点碰撞球放大 EditWaypointGrabRadiusMultiplier 倍"，但用光标射线到
	// 球心的垂距直接算，不真的加大碰撞体：
	//   - 加大碰撞体会挡在 gizmo 轴命中盒前面。GetHitResultUnderCursor 取最近命中，
	//     放大后的球体离相机更近，会把轴拖拽的点击吃掉，精细拖动就废了。
	//   - 这里是独立的一次几何计算，只在常规射线没命中航点后才调用，
	//     对既有 gizmo 拖拽与地形点击零影响。
	// 反投影用 DeprojectMousePositionToWorld（与 ComputeCursorRadiusOnAnchorPlane 同一口径），
	// 全程世界空间，不经过屏幕投影，也就没有 DPI / 视口坐标系的问题。
	...
	ADroneWaypointActor* NearestWaypoint = nullptr;
	// 沿射线取离相机最近的命中，与真实碰撞体的行为一致（近处的挡住远处的）。
	float NearestDistanceAlongRay = TNumericLimits<float>::Max();

	for (const TObjectPtr<ADronePathActor>& PathActor : EditingPaths)
	{
		if (!IsValid(PathActor.Get()))
		{
			continue;
		}
		...
		{
			ADroneWaypointActor* WaypointActor = Waypoint.Get();
			if (!CanInteractWithEditWaypoint(WaypointActor) || !IsValid(WaypointActor->MeshComponent))
			{
				continue;
			}

			// BoxExtent 的最大分量＝球体真实世界半径（已含关卡 2.5 倍与选中 1.5 倍放大）。
			// 不用 Bounds.SphereRadius：它的定义是 BoxExtent.Size()，对球体虚高约 1.73 倍。
			const float WorldRadius = WaypointActor->MeshComponent->Bounds.BoxExtent.GetMax();
			if (WorldRadius <= 0.0f)
			{
				continue;
			}
			const float GrabRadius = WorldRadius * FMath::Max(EditWaypointGrabRadiusMultiplier, 1.0f)
				+ FMath::Max(EditWaypointGrabExtraRadiusCm, 0.0f);

			const FVector OriginToCenter = WaypointActor->GetActorLocation() - RayOrigin;
			const float DistanceAlongRay = FVector::DotProduct(OriginToCenter, RayDirection);
			...
			// 球心到射线的垂距：勾股定理。比较用平方值，省一次开方。
			const float PerpendicularDistanceSquared =
				OriginToCenter.SizeSquared() - (DistanceAlongRay * DistanceAlongRay);
			if (PerpendicularDistanceSquared > GrabRadius * GrabRadius)
			{
				continue;
			}

			if (DistanceAlongRay < NearestDistanceAlongRay)
			{
				NearestDistanceAlongRay = DistanceAlongRay;
				NearestWaypoint = WaypointActor;
			}
		}
	}
...
bool ADroneOpsPlayerController::ResolveRunAnchorDrone(int32& OutDroneId, FVector& OutAnchorWorld)
{
	...
	// 多选路径文件的局部原点固定跟随编号最小的已选无人机，确保同一份
	// JSON 每次派发都有稳定且可预期的编队基准；单选时仍使用主选中无人机。
	TArray<int32> SelectedDroneIds;
	for (const int32 DroneId : DroneRegistry->GetMultiSelectedDrones())
	{
		if (DroneId > 0 && !SelectedDroneIds.Contains(DroneId))
		{
			SelectedDroneIds.Add(DroneId);
		}
	}
	SelectedDroneIds.Sort();
	if (SelectedDroneIds.Num() > 1)
	{
		FVector Loc;
		if (TryResolve(SelectedDroneIds[0], Loc))
		{
			OutDroneId = SelectedDroneIds[0];
			OutAnchorWorld = Loc;
			return true;
		}
		return false;
	...
	if (SelectedDroneIds.Num() == 1)
	{
		const int32 SingleSelectionPrimary = DroneRegistry->GetPrimarySelectedDrone() > 0
			? DroneRegistry->GetPrimarySelectedDrone()
			: SelectedDroneIds[0];
		FVector Loc;
		if (TryResolve(SingleSelectionPrimary, Loc))
		{
			OutDroneId = SingleSelectionPrimary;
			OutAnchorWorld = Loc;
			return true;
		}
		return false;
...
bool ADroneOpsPlayerController::BeginFormationRotatePreview(const FDronePathsSaveData& PathsData)
{
	...
	// 手动编辑路径文件的局部原点始终是 JSON 数组中第一条有效路径的首航点。
	const FDronePathSaveData* FirstPath = PathsData.Paths.FindByPredicate(
	...
	FormationRefEditOrigin = FirstPath->Waypoints[0].Location;
...
{
    if (IsCommandMapViewActive()) { bEditCameraActive = true; return; }
...
{
    if (IsCommandMapViewActive()) { bEditCameraActive = false; bEditCameraLooking = false; return; }
	if (!bEditCameraActive)
	{
		return;
	}
...
void ADroneOpsPlayerController::LoadMissionPath(const FDronePathSaveData& Data,bool Editable) {
    ...
    Path->SetPathNumericId(Data.PathId);Path->SetClosedLoop(Data.bClosedLoop);
