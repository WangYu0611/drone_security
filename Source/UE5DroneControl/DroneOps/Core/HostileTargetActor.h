...
public:
	...
	/**
	 * Set a moving actor that represents this target during playback preview.
	 * While set, GetHostileTargetLocation() returns the representative's location.
	 * Pass nullptr to clear (restores static self-location).
	 */
	UFUNCTION(BlueprintCallable, Category = "HostileTarget")
	void SetMovingRepresentative(AActor* Representative);
...
private:
	...
	/** Optional moving actor that represents this target during playback preview. */
	UPROPERTY()
	TWeakObjectPtr<AActor> MovingRepresentative;
};
