...
/**
 * 无人机外观模型变更时广播（由 SetDroneModelType 触发）。
 * 消费者：两种 Pawn 各自过滤 DroneId 后调用 UDroneVisualComponent::ApplyModelType，
 * 以及列表面板同步下拉框显示。
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnDroneModelTypeChanged,
	int32, DroneId, EDroneModelType, ModelType);
...
public:
	...
	/** 只返回友方无人机（bIsEnemyTarget == false），用于视频流等不应显示敌对目标的列表。 */
	UFUNCTION(BlueprintCallable, Category = "DroneRegistry")
	TArray<FDroneDescriptor> GetFriendlyDroneDescriptors() const;
	...
	/** 外观模型变更时广播（由 SetDroneModelType 触发）。 */
	UPROPERTY(BlueprintAssignable, Category = "DroneRegistry")
	FOnDroneModelTypeChanged OnDroneModelTypeChanged;

	/**
	 * 设置某架无人机的外观模型并广播 OnDroneModelTypeChanged。
	 * 与 LabelSettingsMap 一致，仅内存态，不持久化——重启后回到 Original。
	 */
	UFUNCTION(BlueprintCallable, Category = "DroneRegistry")
	void SetDroneModelType(int32 DroneId, EDroneModelType ModelType);

	/** 查询外观模型。未注册的 DroneId 返回 Original，且不写入 map。 */
	UFUNCTION(BlueprintPure, Category = "DroneRegistry")
	EDroneModelType GetDroneModelType(int32 DroneId) const;
...
private:
	...
	// 每架无人机独立选择的外观模型。缺省 Original，不持久化。
	UPROPERTY()
	TMap<int32, EDroneModelType> ModelTypeMap;
};
