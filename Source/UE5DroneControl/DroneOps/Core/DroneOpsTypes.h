...
namespace DroneOpsConst
{
	/**
	 * DroneId values >= EnemyIdBase are reserved for enemy targets.
	 * Friendly drone IDs must always be < EnemyIdBase.
	 * EnemyTarget DroneId = EnemyIdBase + TargetId.
	 */
	inline constexpr int32 EnemyIdBase = 10000;

	inline bool IsEnemyId(int32 DroneId) { return DroneId >= EnemyIdBase; }
	inline int32 TargetIdToDroneId(int32 TargetId) { return EnemyIdBase + TargetId; }
	inline int32 DroneIdToTargetId(int32 DroneId)  { return DroneId - EnemyIdBase; }
}
...
struct FDroneDescriptor
{
	...
	/** True for enemy targets: local-only, never sent to backend. DroneId must be >= DroneOpsConst::EnemyIdBase. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Drone")
	bool bIsEnemyTarget = false;

	/** World-space spawn location recorded at registration time; used as the first waypoint origin in path-edit mode. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Drone")
	FVector EnemyInitialLocation = FVector::ZeroVector;
