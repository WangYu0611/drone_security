...
{
	if (MovingRepresentative.IsValid())
	{
		return MovingRepresentative->GetActorLocation();
	}
	return GetActorLocation();
}

void AHostileTargetActor::SetMovingRepresentative(AActor* Representative)
{
	MovingRepresentative = Representative;
...
void AHostileTargetActor::UpdateVisuals()
{
	...
	// 有 MovingRepresentative 说明这是挂载在敌对影子机上的动态目标，不显示静态方块和标签
	const bool bIsMovingTarget = MovingRepresentative.IsValid();
	MeshComponent->SetVisibility(!bIsMovingTarget);
	MeshComponent->SetHiddenInGame(bIsMovingTarget);
	if (LabelComponent)
	{
		LabelComponent->SetVisibility(!bIsMovingTarget);
		LabelComponent->SetHiddenInGame(bIsMovingTarget);
	}
