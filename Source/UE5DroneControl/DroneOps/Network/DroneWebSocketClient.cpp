...
void UDroneWebSocketClient::Connect()
{
    ...
    // An old libwebsockets connection may finalize after replacement or PIE teardown.
    if (Socket) {
        Socket->OnConnected().Clear(); Socket->OnConnectionError().Clear();
        Socket->OnClosed().Clear(); Socket->OnMessage().Clear();
...
void UDroneWebSocketClient::BindSocketEvents()
{
	...
	Socket->OnClosed().AddWeakLambda(this, [this](int32 StatusCode, const FString& Reason, bool bWasClean)
	{
	...
	Socket->OnMessage().AddWeakLambda(this, [this](const FString& Message)
	{
...
void UDroneWebSocketClient::ScheduleReconnect()
{
	...
	World->GetTimerManager().SetTimer(ReconnectTimer, FTimerDelegate::CreateWeakLambda(this, [this]()
	{
