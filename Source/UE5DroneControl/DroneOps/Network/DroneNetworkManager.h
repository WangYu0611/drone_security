...
public:
	...
	FString BackendBaseUrl = TEXT("http://192.168.30.111:8080");

	/** WebSocket URL, e.g. "ws://127.0.0.1:8081/ws" */
	UPROPERTY(config, EditAnywhere, BlueprintReadWrite, Category = "Network")
	FString WebSocketUrl = TEXT("ws://192.168.30.111:8081/ws");
     ...
     * @param bCoalesceQueuedVerticalTargets  Replace an obsolete pending Q/E height target with the newest one.
	 * @param SpeedMps  Requested 3D speed in m/s. 0 is the unlimited-speed marker.
	 */
	UFUNCTION(BlueprintCallable, Category = "Network")
	void SendMoveCommand(int32 DroneId, const FVector& TargetWorldLocation,
		EDroneCommandMode Mode = EDroneCommandMode::Move, bool bCoalesceQueuedVerticalTargets = false,
		float SpeedMps = 1.0f, bool bCoalesceQueuedManualTargets = false);
