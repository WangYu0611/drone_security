...
void UDroneNetworkManager::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	FParse::Value(FCommandLine::Get(), TEXT("P1Http="), BackendBaseUrl);
	FParse::Value(FCommandLine::Get(), TEXT("P1Ws="), WebSocketUrl);
...
void UDroneNetworkManager::SyncDroneListToRegistry(const TArray<TSharedPtr<FJsonObject>>& DroneObjects)
{
	...
	{
		// 敌对目标是纯本地构造，不参与后端同步，不应被清理
		if (LocalDesc.bIsEnemyTarget)
		{
			continue;
		}
...
void UDroneNetworkManager::SendMoveCommand(int32 DroneId, const FVector& TargetWorldLocation,
	EDroneCommandMode Mode, bool bCoalesceQueuedVerticalTargets, float SpeedMps, bool bCoalesceQueuedManualTargets)
{
	...
	// { "mode": "move|scout|patrol|attack", "drone_id": "N", "x": ..., "y": ..., "z": ..., "speed": ... }
	...
	// UE -> backend is JSON, so NaN cannot be represented portably. Keep 0 as the
	// unlimited marker; the later backend -> Jetson protocol adapter converts it to NaN.
	const float SanitizedSpeedMps = FMath::IsNearlyZero(SpeedMps)
		? 0.0f
		: FMath::Clamp(SpeedMps, 0.5f, 15.0f);
	const FString Json = FString::Printf(
		TEXT("{\"mode\":\"%s\",\"drone_id\":\"%d\",\"x\":%.2f,\"y\":%.2f,\"z\":%.2f,\"speed\":%.2f,\"coalesce_queued_vertical_targets\":%s,\"coalesce_queued_manual_targets\":%s}"),
		*ProtocolMode,
		DroneId,
		TargetWorldLocation.X, TargetWorldLocation.Y, TargetWorldLocation.Z,
		SanitizedSpeedMps,
		bCoalesceQueuedVerticalTargets ? TEXT("true") : TEXT("false"),
		bCoalesceQueuedManualTargets ? TEXT("true") : TEXT("false"));
