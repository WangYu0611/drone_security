...
namespace DronePathVisual
{
	...
	// 预演使用的缺省速度 (m/s)。0 代表真机不限速，但预演必须有有限时长。
...
ADronePathActor::ADronePathActor()
{
	...
	// PathVisualizationMaterial / PathConflictMaterial are intentionally left null here.
	// Assign a material that exposes Color/BaseColor/EmissiveColor/Tint parameters
	// (e.g. M_DroneHighlight) in the Blueprint Details panel so ApplyColorToMaterial works.
	// If left null, ResolveMaterialInstance will fall back to the mesh's existing material.
...
void ADronePathActor::AddWaypointAtEnd()
{
	...
	// 新增路径点默认沿用上一段速度；若上一段无有效速度或这是首段之后的第一段，使用缺省 5m/s。
...
void ADronePathActor::PrecomputeSegmentDurations()
{
	...
	for (int32 WaypointIndex = 1; WaypointIndex < Waypoints.Num(); ++WaypointIndex)
	{
		...
		const float PreviewSpeedMetersPerSecond = SpeedMetersPerSecond > KINDA_SMALL_NUMBER
			? SpeedMetersPerSecond
			: DronePathVisual::DefaultSegmentSpeedMps;
		const float ComputedDuration = DistanceMeters / PreviewSpeedMetersPerSecond;
...
void ADronePathActor::SnapControlledDroneToPathStart() const
{
	...
	// 敌对无人机从当前位置开始运动，不强制传送到路径起点
	if (const AMultiDroneCharacter* Multi = Cast<AMultiDroneCharacter>(DroneActor))
	{
		if (Multi->bIsEnemyTarget)
		{
			return;
		}
	}
...
UMaterialInstanceDynamic* ADronePathActor::ResolveMaterialInstance(UMeshComponent* PrimitiveComponent, UMaterialInterface* BaseMaterial, TObjectPtr<UMaterialInstanceDynamic>& CachedMID, TObjectPtr<UMaterialInterface>& CachedSource)
{
	...
	// Command's fallback supports spline meshes and keeps color stable under exposure.
	if (!IsValid(BaseMaterial) && MapDisplayState >= 0)
	{
		BaseMaterial = CommandMapMaterial;
	}

	if (!IsValid(BaseMaterial))
	{
		// No explicit material set — create a MID from whatever is already on the mesh
		// so ApplyColorToMaterial can still set parameters on it.
		UMaterialInterface* Existing = PrimitiveComponent->GetMaterial(0);
		if (!IsValid(Existing))
		{
			return nullptr;
		}

		// The component points at CachedMID after the first fallback creation. Do
		// not use that MID as the next MID's parent on every visualization refresh:
		// repeated RefreshConflictVisualization calls would otherwise create a
		// MID_MID_MID... chain until the generated FName exceeds UE's 1023 limit.
		if (IsValid(CachedMID) && Existing == CachedMID.Get())
		{
			return CachedMID;
		}

		// Conflict and non-conflict visuals have separate caches but share the
		// component material slot. When switching between them, unwrap a prior
		// MID to its base material before creating the other cache's MID.
		UMaterialInterface* MaterialSource = Existing;
		if (UMaterialInstanceDynamic* ExistingMID = Cast<UMaterialInstanceDynamic>(Existing))
		{
			if (UMaterial* ResolvedBaseMaterial = ExistingMID->GetMaterial())
			{
				MaterialSource = ResolvedBaseMaterial;
			}
		}

		if (!IsValid(CachedMID) || CachedSource != MaterialSource)
		{
			CachedMID = UMaterialInstanceDynamic::Create(MaterialSource, PrimitiveComponent);
			CachedSource = MaterialSource;
