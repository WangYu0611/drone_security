...
void ADronePlaybackManager::PlayFromData(const FDronePathsSaveData& Data)
{
	...
	for (int32 PathIndex = 0; PathIndex < PlaybackData.Paths.Num(); ++PathIndex)
	{
		...
		// Enemy paths (PathId >= EnemyIdBase) always spawn a dedicated enemy actor
		// and hide the corresponding AHostileTargetActor during playback.
		if (DroneOpsConst::IsEnemyId(PathData.PathId))
		{
			DroneActor = SpawnEnemyPlaybackDrone(StartLocation);
			if (!IsValid(DroneActor))
			{
				PathActor->Destroy();
				continue;
			}
			// Find the matching AHostileTargetActor and wire up MovingRepresentative.
			const int32 TargetId = DroneOpsConst::DroneIdToTargetId(PathData.PathId);
			for (TActorIterator<AHostileTargetActor> It(World); It; ++It)
			{
				if (IsValid(*It) && (*It)->TargetId == TargetId)
				{
					(*It)->SetMovingRepresentative(DroneActor);
					(*It)->SetActorHiddenInGame(true);
					ActiveEnemyHostileActors.Add(PathData.PathId, *It);
					break;
				}
			}
		}
		else if (bUseExistingShadowDrones)
		{
...
void ADronePlaybackManager::ClearActivePlaybackActors()
{
	...
	// Restore all hostile target actors that were hidden during enemy playback.
	for (auto& Pair : ActiveEnemyHostileActors)
	{
		if (IsValid(Pair.Value))
		{
			Pair.Value->SetMovingRepresentative(nullptr);
			Pair.Value->SetActorHiddenInGame(false);
		}
	}
	ActiveEnemyHostileActors.Reset();
...
AActor* ADronePlaybackManager::SpawnEnemyPlaybackDrone(const FVector& SpawnLocation)
{
	UWorld* World = GetWorld();
	if (!IsValid(World))
	{
	...
	TSubclassOf<AActor> ClassToSpawn = EnemyDroneActorClass ? EnemyDroneActorClass : DroneActorClass;
	if (ClassToSpawn)
	{
		AActor* SpawnedActor = World->SpawnActor<AActor>(ClassToSpawn, SpawnLocation, FRotator::ZeroRotator, SpawnParameters);
		if (SpawnedActor)
		{
			if (AMultiDroneCharacter* Multi = Cast<AMultiDroneCharacter>(SpawnedActor))
			{
	...
	// Fallback: reuse the same skeletal mesh as the friendly fallback but tinted red via a dynamic material.
	// Full material swap is handled in BP_EnemyDroneCharacter; here we just ensure something is visible.
	return SpawnPlaybackDrone(SpawnLocation);
}
...
void ADronePlaybackManager::StopPlaybackAndClearPreviewInWorld(UWorld* World)
{
	if (!IsValid(World))
	{
		return;
	...
	// 1. 停掉所有 playback manager。其 StopPlayback 会销毁自己 ActivePaths 里追踪的
	//    预览路径（正常播放路径），但不动编辑路径。
	for (TActorIterator<ADronePlaybackManager> It(World); It; ++It)
	{
		if (IsValid(*It))
	...
	// 2. 兜底清理"漏网的预览路径"：只销毁 Owner 为 ADronePlaybackManager 的路径 Actor
	//    （如卡在循环中、或播放/停止分属不同实例导致未被上面清掉的预览路径）。
	//    编辑用路径（如 DronePathManager 创建的初始 6 条，其 Owner 不是播放管理器）
	//    会被此判断跳过，从而保留下来。
	for (TActorIterator<ADronePathActor> It(World); It; ++It)
	{
		ADronePathActor* PathActor = *It;
		if (IsValid(PathActor) && PathActor->GetOwner() && PathActor->GetOwner()->IsA<ADronePlaybackManager>())
		{
			PathActor->StopMovement();
			PathActor->Destroy();
		}
