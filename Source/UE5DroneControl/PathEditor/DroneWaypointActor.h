...
public:
	...
	/**
	 * 仅在 CesiumWorld 预演关卡对路点球体额外放大的倍数。
	 * CesiumWorld 相机常拉远，默认球体(≈15cm)在屏幕上过小难以点选；此倍数只在
	 * CesiumWorld 生效，EditMap 等其他关卡保持 1.0 不变。BeginPlay 时按当前地图名决定。
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Waypoint|Visualization", meta = (ClampMin = "1.0", UIMin = "1.0"))
	float CesiumWorldScaleMultiplier = 2.5f;
...
private:
	...
	// 当前关卡对路点球体的额外缩放：CesiumWorld 取 CesiumWorldScaleMultiplier，其余关卡为 1.0。
	// 在 BeginPlay 里按地图名解析，供 ApplyVisualState 使用（避免每帧查地图名）。
	float ActiveMapScaleMultiplier = 1.0f;
	float CommandMapScaleMultiplier = 1.0f;
	float MissionGizmoScale = 1.0f;
	bool bCommandMapVisualsApplied = false;
	float OriginalTextWorldSize = 48.f;
	FVector OriginalTextRelativeLocation = FVector::ZeroVector;
