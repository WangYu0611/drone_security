...
void ADroneWaypointActor::BeginPlay()
{
	...
	// 仅在 CesiumWorld 预演关卡放大球体：该关卡相机常拉远，默认 15cm 球在屏幕上过小难点选。
	// 其余关卡（如 EditMap 编辑关卡）保持原尺寸，避免近距离编辑时球体过大遮挡。
	ActiveMapScaleMultiplier = 1.0f;
	if (const UWorld* World = GetWorld())
	{
		if (World->GetMapName().Contains(TEXT("CesiumWorld")))
		{
			ActiveMapScaleMultiplier = FMath::Max(1.0f, CesiumWorldScaleMultiplier);
		}
	}
...
void ADroneWaypointActor::UpdateTextFacingCamera()
{
	...
	const auto* CommandController = Cast<ADroneOpsPlayerController>(PlayerController);
	if (CommandController && CommandController->IsCommandMapViewActive())
	{
		int32 Width = 0, Height = 0;
		PlayerController->GetViewportSize(Width, Height);
		if (Width <= 0) return;
		if (!bCommandMapVisualsApplied)
		{
			OriginalTextWorldSize = TextRenderComponent->WorldSize;
			OriginalTextRelativeLocation = TextRenderComponent->GetRelativeLocation();
			bCommandMapVisualsApplied = true;
			UpdateDisplayText(DisplayPathNumber, DisplaySegmentSpeed);
		}
		...
		const FVector Forward = CameraAxes.GetUnitAxis(EAxis::X), Up = CameraAxes.GetUnitAxis(EAxis::Z);
		const double Depth = FMath::Max(1.0, FVector::DotProduct(GetActorLocation() - Camera->GetCameraLocation(), Forward));
		const float UnitsPerPixel = static_cast<float>(2.0 * Depth * FMath::Tan(FMath::DegreesToRadians(Camera->GetFOVAngle() * .5)) / Width);
		CommandMapScaleMultiplier = FMath::Max(1.f, UnitsPerPixel * 6.f / (50.f * DroneWaypointVisual::BaseWaypointMeshScale * ActiveMapScaleMultiplier));
		// P3 mission editing uses the existing axes. Keep handles and hit targets
		// usable at map scale without moving the waypoint or changing saved data.
		const float NewGizmoScale = CommandController->IsMissionPathMode()
			? FMath::Max(1.f, UnitsPerPixel * 48.f / FMath::Max(GizmoHandleLength, 1.f)) : 1.f;
		if (!FMath::IsNearlyEqual(MissionGizmoScale, NewGizmoScale))
		{
			MissionGizmoScale = NewGizmoScale;
			ApplyVisualState();
		}
		if (MeshComponent)
			MeshComponent->SetRelativeScale3D(FVector(DroneWaypointVisual::BaseWaypointMeshScale * ActiveMapScaleMultiplier * CommandMapScaleMultiplier * (bSelected ? SelectedScaleMultiplier : 1.f)));
		TextRenderComponent->SetWorldSize(FMath::Max(OriginalTextWorldSize, UnitsPerPixel * 16.f));
		TextRenderComponent->SetWorldLocation(GetActorLocation() + Up * UnitsPerPixel * 12.f - Forward * UnitsPerPixel * 8.f);
		TextRenderComponent->SetWorldRotation(FRotationMatrix::MakeFromXZ(-Forward, Up).Rotator());
		return;
	}
	if (bCommandMapVisualsApplied)
	{
		bCommandMapVisualsApplied = false;
		UpdateDisplayText(DisplayPathNumber, DisplaySegmentSpeed);
		CommandMapScaleMultiplier = 1.f;
		MissionGizmoScale = 1.f;
		TextRenderComponent->SetWorldSize(OriginalTextWorldSize);
		TextRenderComponent->SetRelativeLocation(OriginalTextRelativeLocation);
		ApplyVisualState();
	}
...
void ADroneWaypointActor::ApplyVisualState()
{
	...
	auto UpdateGizmoHandle = [&](UArrowComponent* GizmoHandle, UBoxComponent* GizmoHitTarget, const FLinearColor& BaseColor, const FVector& AxisDirection, const EGizmoAxis Axis)
	{
		...
		GizmoHandle->SetRelativeLocation(AxisDirection * DroneWaypointVisual::GizmoHandleBaseOffsetCm * MissionGizmoScale);
		GizmoHandle->SetRelativeRotation(AxisDirection.Rotation());
		GizmoHandle->SetRelativeScale3D(FVector(AxisScale * MissionGizmoScale));
		...
		GizmoHitTarget->SetRelativeLocation(AxisDirection * CollisionCenterOffset * MissionGizmoScale);
		GizmoHitTarget->SetRelativeRotation(AxisDirection.Rotation());
		GizmoHitTarget->SetBoxExtent(FVector(CollisionHalfLength, CollisionRadius * AxisScale, CollisionRadius * AxisScale) * MissionGizmoScale);
	};
