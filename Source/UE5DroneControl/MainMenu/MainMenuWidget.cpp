...
{
	// Friendly drone IDs must not overlap the enemy-reserved range.
	if (DroneOpsConst::IsEnemyId(DroneId))
	{
		UE_LOG(LogTemp, Warning, TEXT("[MainMenu] DroneId %d is in the enemy-reserved range (>= %d). Use RegisterEnemyTarget instead."),
			DroneId, DroneOpsConst::EnemyIdBase);
		return;
	}
...
void UMainMenuWidget::RegisterEnemyTarget(int32 TargetId, const FVector& InitialWorldLocation)
{
	if (TargetId <= 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("[MainMenu] RegisterEnemyTarget: invalid TargetId %d"), TargetId);
		return;
	}
	...
	const int32 DroneId = DroneOpsConst::TargetIdToDroneId(TargetId);

	FDroneDescriptor Desc;
	Desc.DroneId             = DroneId;
	Desc.Slot                = DroneId;
	Desc.Name                = FString::Printf(TEXT("Enemy-%d"), TargetId);
	Desc.bIsEnemyTarget      = true;
	Desc.EnemyInitialLocation = InitialWorldLocation;
	// Network fields intentionally left empty — enemy targets are local-only.

	Registry->RegisterDrone(Desc);
	// Enemy targets have no real connectivity; mark Offline so the UI does not show them as Lost.
	Registry->MarkDroneAvailability(DroneId, EDroneAvailability::Offline);
	OnDroneRegistered(DroneId);

	UE_LOG(LogTemp, Log, TEXT("[MainMenu] Registered enemy target T-%d as DroneId=%d"), TargetId, DroneId);
}
...
TArray<FString> UMainMenuWidget::GetRegisteredDronesSummary() const
{
	...
	for (const FDroneDescriptor& Desc : Registry->GetAllDroneDescriptors())
	{
		if (Desc.bIsEnemyTarget)
		{
			...
			Result.Add(FString::Printf(TEXT("【敌对】T-%d  %s"), TargetId, *Desc.Name));
		}
		else
		{
...
void UMainMenuWidget::LoadSettings(FString& OutBackendAddress, float& OutMapCenterLat, float& OutMapCenterLon, float& OutMapCenterAlt, bool& OutbUseCesium) const
{
	...
	// 默认值
	OutBackendAddress = TEXT("ws://192.168.30.111:8081/ws");
...
void UMainMenuWidget::SetGroundProjectionRayDensity(float NormalizedDensity)
{
	UDroneGroundProjectionComponent::SetGroundProjectionDensity(this, NormalizedDensity);
}

float UMainMenuWidget::GetGroundProjectionRayDensity() const
{
	return UDroneGroundProjectionComponent::GetGroundProjectionDensity();
}
