...
public:
	...
	/**
	 * 注册一个敌对目标到 DroneRegistrySubsystem（纯本地，不发往后端）。
	 * DroneId 将被设为 DroneOpsConst::EnemyIdBase + TargetId。
	 * @param TargetId            1-based 目标编号（由 UI 输入）
	 * @param InitialWorldLocation 初始世界坐标（路径编辑起点），零向量时由编辑模式相机位置补全
	 */
	UFUNCTION(BlueprintCallable, Category = "MainMenu|DroneRegistry")
	void RegisterEnemyTarget(int32 TargetId, const FVector& InitialWorldLocation);
	...
	/** Save the normalized projection density (0 = sparse, 1 = maximum). */
	UFUNCTION(BlueprintCallable, Category = "MainMenu|Settings")
	void SetGroundProjectionRayDensity(float NormalizedDensity);

	/** Read the normalized projection density. */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "MainMenu|Settings")
	float GetGroundProjectionRayDensity() const;
