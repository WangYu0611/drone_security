...
BACKEND_HOST = os.environ.get("BACKEND_HOST", "192.168.30.111").strip()
...
MIN_LIMITED_SPEED_MPS = 0.5
MAX_LIMITED_SPEED_MPS = 15.0
# When a speed-limited vector reaches this distance from the target, switch
# back to position hold so the vehicle settles instead of flying past it.
SPEED_ARRIVAL_RADIUS_M = float(
    os.environ.get("SPEED_ARRIVAL_RADIUS_M", "0.25")
)
...
def parse_speed_mps(raw_speed):
    """Parse the optional command speed.

    A finite value is the requested *3D vector magnitude* in m/s and must be
    within 0.5–15.  ``"NaN"`` is the JSON-safe unlimited-speed marker used by
    the backend; numeric 0 is accepted as a backward-compatible alias.  A
    missing field also preserves the legacy position-control behaviour.
    """
    if raw_speed is None:
        return float("nan")
    if isinstance(raw_speed, bool):
        raise ValueError("speed must be a number or the string 'NaN'")
    if isinstance(raw_speed, str):
        if raw_speed.strip().lower() == "nan":
            return float("nan")
        raise ValueError("speed string must be exactly 'NaN'")
    try:
        speed_mps = float(raw_speed)
    except (TypeError, ValueError, OverflowError) as exc:
        raise ValueError(f"invalid speed: {raw_speed!r}") from exc
    if not math.isfinite(speed_mps):
        raise ValueError("finite speed must not be NaN/Infinity; use string 'NaN'")
    if abs(speed_mps) <= 1e-6:
        return float("nan")
    if not MIN_LIMITED_SPEED_MPS <= speed_mps <= MAX_LIMITED_SPEED_MPS:
        raise ValueError(
            f"speed must be 0 (unlimited) or {MIN_LIMITED_SPEED_MPS:g}-"
            f"{MAX_LIMITED_SPEED_MPS:g} m/s, got {speed_mps}"
        )
    return speed_mps


def format_speed_mps(speed_mps: float) -> str:
    """Human-readable speed for diagnostics without emitting a JSON NaN."""
    return "unlimited" if not math.isfinite(speed_mps) else f"{speed_mps:.2f}m/s"
...
def parse_control_packet(data: bytes):
    ...
    speed_mps = parse_speed_mps(message.get("speed"))
...
class JetsonBridge(Node):
    def __init__(self, slot: int = 1):
        ...
        if (
            not math.isfinite(SPEED_ARRIVAL_RADIUS_M)
            or SPEED_ARRIVAL_RADIUS_M <= 0.0
        ):
            raise ValueError("SPEED_ARRIVAL_RADIUS_M must be finite and > 0")
        ...
        # PX4 v1 messages retain the VehicleLocalPosition type, but expose it
        # on the versioned uXRCE-DDS topic.
        self.create_subscription(
            VehicleLocalPosition,
            f"{self._topic_prefix}/fmu/out/vehicle_local_position_v1",
        ...
        # PX4 v1: battery_status_v1
        self.create_subscription(
            BatteryStatus,
            f"{self._topic_prefix}/fmu/out/battery_status_v1",
        ...
        self._ping_rx_count = 0
        self._pong_tx_errors = 0
    ...
    def _speed_limited_velocity(self, sp: dict):
        """Return a NED velocity vector for a finite-speed move, or ``None``.

        ``None`` means either an unlimited/hold command or that the target is
        already inside the arrival radius.  The vector magnitude is clamped to
        the remaining distance per 50 Hz cycle, preventing the final command
        from stepping past the waypoint in the ideal kinematic case.
        """
        speed_mps = float(sp.get("speed_mps", float("nan")))
        if sp.get("mode") != "move" or not math.isfinite(speed_mps):
            return None

        current_ned = valid_vehicle_local_ned(self._local_pos)
        if current_ned is None:
            # _on_local_pos clears the cached setpoint when the estimator is
            # invalid. Keep this defensive fallback for a race between callbacks.
            return None

        delta_north = sp["x"] - current_ned[0]
        delta_east = sp["y"] - current_ned[1]
        delta_down = sp["z"] - current_ned[2]
        distance_m = math.sqrt(
            delta_north * delta_north
            + delta_east * delta_east
            + delta_down * delta_down
        )
        if distance_m <= SPEED_ARRIVAL_RADIUS_M:
            return None

        # The requested value is the total 3D speed, not horizontal speed.
        commanded_speed_mps = min(speed_mps, distance_m / OFFBOARD_INTERVAL)
        scale = commanded_speed_mps / distance_m
        return [
            delta_north * scale,
            delta_east * scale,
            delta_down * scale,
    ...
    def _offboard_loop(self):
        ...
        # A finite speed uses velocity-only control. The direction is recomputed
        # from PX4's current local NED position at 50 Hz, so its 3D magnitude is
        # exactly the requested segment speed. NaN preserves legacy position
        # control (unlimited speed). Once inside the arrival radius, position
        # hold settles at the requested target.
        velocity_setpoint = self._speed_limited_velocity(sp)
        using_speed_limited_velocity = velocity_setpoint is not None

        # 1. 持续发布 OffboardControlMode
        ocm = OffboardControlMode()
        ocm.timestamp = now_us
        ocm.position = not using_speed_limited_velocity
        ocm.velocity = using_speed_limited_velocity
        ...
        if using_speed_limited_velocity:
            tsp.position = [float("nan")] * 3
            tsp.velocity = velocity_setpoint
        else:
    ...
    # ------------------------------------------------------------------
    # 延迟探测 ping/pong（不进入 parse_control_packet 的严格协议校验，
    # 不占用 command_id/sequence，不影响 move/hold 状态机）
    # ------------------------------------------------------------------
    def _maybe_reply_ping(self, data: bytes, addr) -> bool:
        """若为延迟探测 ping 包，立即原样回 pong 并返回 True；否则返回 False。"""
        ...
        # RTT 完全由后端自身单调时钟计算；这里回显 sent_at_unix_s 仅供
        # 后端日志参考 Jetson 与后端的系统时钟差，不参与延迟计算，
        # 因此两端时钟不同步不影响 delay-test 的结果。
        pong = {
            "protocol": CONTROL_PROTOCOL,
            "version": CONTROL_PROTOCOL_VERSION,
            "type": "pong",
            "ping_id": ping_id,
            "sent_at_unix_s": message.get("sent_at_unix_s"),
            "jetson_recv_at_unix_s": time.time(),
        }
        try:
            payload = json.dumps(pong).encode("utf-8")
            self._tel_sock.sendto(payload, (addr[0], self._tel_port))
        except Exception as e:
            self.get_logger().error(
                f"[PING] pong reply to {addr[0]}:{self._tel_port} failed: "
                f"{type(e).__name__}: {e}"
            )
            return True

        self._ping_rx_count += 1
        if self._ping_rx_count == 1 or self._ping_rx_count % 20 == 0:
            self.get_logger().info(
                f"[PING] replied pong #{self._ping_rx_count} ping_id={ping_id} "
                f"to {addr[0]}:{self._tel_port}"
            )
        return True
    ...
    def process_control(self):
        ...
        if self._maybe_reply_ping(data, addr):
            return
        ...
        if should_log:
            self.get_logger().info(
                ...
                f"speed={format_speed_mps(parsed['speed_mps'])} "
    ...
    def _control_fingerprint(parsed: dict):
        ...
        # NaN is deliberately not self-equal, so normalize the unlimited marker
        # before comparing repeated packets from the same command.
        speed_fingerprint = (
            None
            if not math.isfinite(parsed["speed_mps"])
            else parsed["speed_mps"]
        )
        return (
            parsed["session_id"], parsed["command_id"], parsed["sequence"],
            parsed["drone_id"], parsed["slot"], parsed["mode"],
            parsed["x"], parsed["y"], parsed["z"], speed_fingerprint,
        )
