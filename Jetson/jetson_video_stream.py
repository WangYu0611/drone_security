...
Default LAN topology:
  Jetson:   192.168.30.103
  MediaMTX: 192.168.30.111
  Backend:  http://192.168.30.111:8080
  RTSP:     rtsp://192.168.30.111:8554/drone-1
  UE page:  http://192.168.30.111:8889/drone-1
...
def probe_rtsp_server(rtsp_url: str, timeout_seconds: float) -> None:
    """Fail quickly when the RTSP control port cannot be reached.

    RTSP always uses TCP for its control connection, even when media is sent
    with RTP/UDP.  This small probe prevents the GStreamer pipeline from being
    reported as active while it is only waiting on an unreachable server.
    """
...
def next_monotonic_pts_ns(
    clock_ns: int,
    first_clock_ns: int,
    previous_pts_ns: int,
) -> tuple[int, int]:
    """Return a non-decreasing video PTS based on the local monotonic clock.

    Camera Header timestamps remain in FrameTracking for SfM matching.  They
    are deliberately not used for network-video PTS: camera clocks can jump
    or repeat and cause H.264 frames to be rejected as reordered downstream.
    """
    if first_clock_ns <= 0:
        first_clock_ns = clock_ns
    pts_ns = max(0, clock_ns - first_clock_ns)
    if previous_pts_ns >= 0:
        pts_ns = max(pts_ns, previous_pts_ns + 1)
    return first_clock_ns, pts_ns
...
def build_pipeline_description(
    ...
    rtsp_branch = (
        ...
        "leaky=downstream "
        f"! rtspclientsink location={gst_quote(config.rtsp_url)} "
        f"protocols={config.rtsp_transport}"
    )
...
class GstStreamPipeline:
    ...
    def __init__(self, config: StreamConfig):
        ...
        self.encoding = ""
        self.generation = 0
        self.first_video_clock_ns = 0
        self.last_stream_pts_ns = -1
    ...
    def _start(self, frame: FrameTracking) -> None:
        ...
        probe_rtsp_server(
            ...
            self.config.rtsp_connect_timeout_seconds,
        )
        self.stop()
        self.generation += 1
        ...
        while True:
            ...
            if (
                ...
                if new == Gst.State.PLAYING:
                    # PLAYING means frames can be sent to rtspclientsink.  It
                    # does not prove that MediaMTX has already accepted them.
    ...
    def push(self, packet: FramePacket) -> tuple[bool, int, int, str, str]:
        ...
        try:
            ...
            self.first_video_clock_ns, pts = next_monotonic_pts_ns(
                clock_ns,
                self.first_video_clock_ns,
                self.last_stream_pts_ns,
            )
            if self.last_stream_pts_ns < 0:
                duration = int(1_000_000_000 / self.config.fps)
            else:
                duration = max(1, pts - self.last_stream_pts_ns)
            self.last_stream_pts_ns = pts
            buffer.pts = pts
            buffer.dts = pts
...
class JetsonVideoStreamNode(Node):  # type: ignore[misc]
    def __init__(self, args: argparse.Namespace):
        ...
        stream_config = StreamConfig(
            ...
            rtsp_connect_timeout_seconds=args.rtsp_connect_timeout,
        ...
        self.get_logger().info(
            f"RTSP target: {args.rtsp_url} ({args.rtsp_transport}); "
            f"UE page: {args.video_url}"
        )
        if self.local_capture_enabled:
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--jetson-ip", default="192.168.30.103")
    parser.add_argument("--mediamtx-host", default="192.168.30.111")
    ...
    parser.add_argument(
        ...
        default="tcp",
        help=(
            "RTP transport from Jetson to MediaMTX; tcp is the compatibility "
            "default, udp avoids TCP latency accumulation on a clean LAN"
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="seconds to wait for the MediaMTX RTSP control port before retrying",
    )
    ...
    parser.add_argument(
        "--backend-base-url", default="http://192.168.30.111:8080"
    )
...
def finalize_arguments(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.rtsp_connect_timeout <= 0:
        raise ValueError("RTSP connect timeout must be positive")
