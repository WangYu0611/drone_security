...
class TopicAndArgumentsTest(unittest.TestCase):
    ...
    def test_default_lan_urls_and_camera_topics(self):
        ...
        self.assertEqual(args.rtsp_url, "rtsp://192.168.30.111:8554/drone-1")
        self.assertEqual(args.video_url, "http://192.168.30.111:8889/drone-1")
        self.assertEqual(
            args.metadata_endpoint,
            "http://192.168.30.111:8080/api/video-metadata/batch",
        )
        ...
        self.assertEqual(args.rtsp_connect_timeout, 2.0)
...
class PipelineDescriptionTest(unittest.TestCase):
    def test_pipeline_uses_hardware_encoder_rtsp_tcp_and_local_segments(self):
        config = MODULE.StreamConfig(
            ...
            rtsp_transport="tcp",
            rtsp_connect_timeout_seconds=2.0,
            bitrate=8_000_000,
            fps=30,
    ...
    def test_pipeline_can_use_udp_rtp_transport(self):
        config = MODULE.StreamConfig(
            rtsp_url="rtsp://192.168.10.30:8554/drone-1",
            rtsp_transport="udp",
            rtsp_connect_timeout_seconds=2.0,
        ...
        pipeline = MODULE.build_pipeline_description(
            config, "RGB", 640, 480, generation=1
        )
        self.assertIn("protocols=udp", pipeline)

    def test_video_pts_uses_monotonic_clock_and_clamps_backwards_time(self):
        first, pts0 = MODULE.next_monotonic_pts_ns(1_000, 0, -1)
        self.assertEqual(first, 1_000)
        self.assertEqual(pts0, 0)
        first, pts1 = MODULE.next_monotonic_pts_ns(1_040, first, pts0)
        self.assertEqual(pts1, 40)
        _first, pts2 = MODULE.next_monotonic_pts_ns(900, first, pts1)
        self.assertEqual(pts2, 41)
